...
<template>
  ...
  >
    ...
    <span class="voice-label">{{
      voiceLoading ? "加载中" : voicePlaying ? "播放中" : "播放语音"
    }}</span>
    ...
    <span class="voice-sec" v-if="voiceDuration && !voicePlaying">{{
      voiceDuration
    }}</span>
  </div>
</template>
...
.voice-wx-icon .voice-body {
  fill: var(--ac-color-text-secondary);
  transition: fill 0.2s;
}
.voice-bar.playing .voice-wx-icon .voice-body {
  fill: var(--ac-color-primary);
}

.voice-wx-icon .voice-wave {
  opacity: 0.3;
  transition: opacity 0.15s;
}
.voice-bar.playing .voice-wx-icon .voice-wave {
  animation: wavePulse 0.8s ease-in-out infinite;
}
.voice-bar.playing .voice-wx-icon .voice-wave.w2 {
  animation-delay: 0.15s;
}
.voice-bar.playing .voice-wx-icon .voice-wave.w3 {
...
.voice-anim-dots .dot:nth-child(2) {
  animation-delay: 0.15s;
}
.voice-anim-dots .dot:nth-child(3) {
  animation-delay: 0.3s;
}
...
.voice-dots .vdot:nth-child(1) {
  height: 8px;
}
.voice-dots .vdot:nth-child(2) {
  height: 12px;
}
.voice-dots .vdot:nth-child(3) {
  height: 6px;
}
.voice-dots .vdot:nth-child(4) {
  height: 14px;
}
.voice-dots .vdot:nth-child(5) {
  height: 10px;
}
...
.voice-bar.voice-only {
  margin-top: 0;
  padding: 12px 18px;
  min-width: 160px;
  border-radius: 20px;
}
.voice-bar.voice-only .voice-label {
  font-size: 14px;
}
.voice-bar.voice-only .voice-dots {
  margin-left: 8px;
}
</style>
