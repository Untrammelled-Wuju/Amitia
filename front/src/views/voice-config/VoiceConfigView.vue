  ...
  <div class="page">
      ...
      >
        <div class="card-top">
          <div class="card-header">
            ...
            <el-tag
              ...
              >默认</el-tag
            >
            <el-tag v-if="cfg.emotion" size="small" type="warning">{{
              emotionLabel(cfg.emotion)
            }}</el-tag>
          </div>
          <div class="card-type">
        ...
        <div class="card-details">
          <div class="detail-row">
            <span class="dl">资源</span
            ><span class="dv">{{ cfg.resourceId || "seed-tts-2.0" }}</span>
          </div>
          <div class="detail-row">
            <span class="dl">API Key</span
            ><span class="dv">{{ cfg.hasApiKey ? "已设置" : "未设置" }}</span>
          </div>
          <div class="detail-row" v-if="cfg.lastTestResult">
            ...
            ><span
              ...
              >{{
                cfg.lastTestResult === "success" ? "连接正常" : "连接失败"
              }}</span
            >
          </div>
          <div class="detail-row">
            <span class="dl">语速</span
            ><span class="dv">{{ cfg.speed?.toFixed(1) ?? "1.0" }}x</span>
          </div>
          <div class="detail-row">
            <span class="dl">音调</span
            ><span class="dv">{{ cfg.pitch?.toFixed(1) ?? "1.0" }}x</span>
          </div>
          <div class="detail-row">
            <span class="dl">音量</span
            ><span class="dv">{{ Math.round((cfg.volume ?? 1) * 100) }}%</span>
          </div>
        </div>
        <div class="card-actions">
          <el-popover placement="top" :width="280" trigger="click">
            <template #reference>
              <el-button size="small" :loading="testingId === cfg.id"
                >试听</el-button
              >
            </template>
            <div>
    ...
    <div class="clone-section" v-if="configs.length > 0">
      ...
      <p class="section-desc">
        上传一段10-30秒的清晰语音样本，创建你自己的专属音色。创建后可像预设音色一样使用。<br /><strong
          style="color: var(--el-color-warning)"
          >⚠️ 音色首次合成即转正收费，请务必试听满意后再正式使用。</strong
        >
      </p>
      <div class="clone-cards">
        <div v-for="v in clonedVoices" :key="v.speakerId" class="clone-card">
          ...
          <div class="clone-card-actions">
            <el-button
              size="small"
              @click="previewClone(v.speakerId)"
            ...
            <el-button
              size="small"
              type="danger"
              @click="deleteClone(v.speakerId, v.name)"
              >删除</el-button
            >
          </div>
        </div>
    ...
    >
      <el-form :model="cloneForm" label-position="top">
        ...
        <el-form-item label="参考文本（可选）">
          <el-input
            ...
            type="textarea"
            :rows="2"
            placeholder="音频中说的话语文本，用于提升复刻质量"
          />
        </el-form-item>
        <el-form-item label="上传音频">
          ...
          >
            ...
            <template #tip>
              <div class="form-hint">
                支持 mp3/wav/ogg/m4a/aac/pcm，最大10MB。10-30秒清晰语音效果最佳
              </div>
            </template>
          </el-upload>
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button
          ...
          :disabled="!cloneForm.audioFile || !cloneForm.name.trim()"
          >开始复刻</el-button
        >
      </template>
    </el-dialog>
      ...
      >
        ...
        <el-form-item label="API Key" prop="apiKey">
          <el-input
            ...
            show-password
            :placeholder="
              editingId ? '留空则保留现有 Key' : '火山引擎控制台 API Key'
            "
          />
        </el-form-item>
...
function showDialog(cfg: TtsConfig | null) {
  ...
  if (cfg) {
    ...
    form.voiceType = cfg.voiceType;
    form.emotion = cfg.emotion || "";
    form.speed = cfg.speed;
    form.pitch = cfg.pitch;
    form.volume = cfg.volume;
  } else {
    editingId.value = null;
    form.name = "";
    form.apiKey = "";
    form.voiceType = "zh_female_cancan_mars_bigtts";
    form.emotion = "";
    form.speed = 1.0;
    form.pitch = 1.0;
    form.volume = 1.0;
