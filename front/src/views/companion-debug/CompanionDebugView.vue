  ...
  <div class="debug-panel">
    <div class="page-header">
      ...
      <el-button type="primary" @click="loadAll" :loading="loading"
        >刷新</el-button
      >
      <el-button type="warning" @click="regenerateAll" :loading="regenerating"
        >重新生成全部</el-button
      >
      <el-button
        type="success"
        @click="triggerDailyRegen"
    ...
    <el-card
      ...
      shadow="never"
      v-loading="temporalLoading"
    >
      <template #header
        ><div class="temporal-header">
          <span>Temporal Runtime</span>
          <div class="temporal-badges">
            <el-tag size="small">{{
              temporalDiagnostics?.snapshotVersion || "未加载"
            }}</el-tag
            ><el-tag
              size="small"
              :type="relationshipFeatureEnabled ? 'success' : 'info'"
              >Relationship Time
              {{ relationshipFeatureEnabled ? "已启用" : "未启用" }}</el-tag
            >
          </div>
        </div></template
      >
      <el-alert
        ...
        show-icon
        :closable="false"
        class="temporal-alert"
        ><el-button @click="loadTemporal">重试时间诊断</el-button></el-alert
      >
      <el-tabs v-else-if="temporalDiagnostics" v-model="temporalActiveTab">
        <el-tab-pane label="Core Time" name="core">
          <el-descriptions :column="2" border size="small">
            <el-descriptions-item label="用户当地时间">{{
              formatTemporalCivil(temporalDiagnostics.snapshot.userTime)
            }}</el-descriptions-item>
            <el-descriptions-item label="角色当地时间">{{
              formatTemporalCivil(temporalDiagnostics.snapshot.characterTime)
            }}</el-descriptions-item>
            <el-descriptions-item label="时区是否不同">{{
              temporalDiagnostics.snapshot.signals.timezoneDiffers ? "是" : "否"
            }}</el-descriptions-item>
            <el-descriptions-item label="用户安静时段">{{
              temporalDiagnostics.snapshot.signals.quietHours ? "是" : "否"
            }}</el-descriptions-item>
            <el-descriptions-item label="Clock 来源">{{
              temporalDiagnostics.core.clockSource
            }}</el-descriptions-item>
            <el-descriptions-item label="TZDB">{{
              temporalDiagnostics.core.tzdb
            }}</el-descriptions-item>
          </el-descriptions>
        </el-tab-pane>
        <el-tab-pane label="Anchors" name="anchors">
          <el-table
            :data="temporalDiagnostics.snapshot.salientAnchors || []"
            size="small"
            stripe
          >
            <el-table-column
            ...
            /><el-table-column
              ...
              label="距离天数"
              width="100"
            /><el-table-column prop="salience" label="显著性" width="90" />
          </el-table>
          <el-empty
            v-if="!temporalDiagnostics.snapshot.salientAnchors?.length"
            description="当前没有显著时间锚点"
            :image-size="42"
          />
        </el-tab-pane>
        <el-tab-pane label="Relationship Time" name="relationship">
          <el-alert
            v-if="!relationshipFeatureEnabled"
            title="Relationship Time 后端功能开关未启用"
            description="当前不会生成关系时间状态或影响角色表达。"
            type="info"
            show-icon
            :closable="false"
          />
          <template v-else-if="relationshipContext">
            <el-descriptions :column="2" border size="small">
              <el-descriptions-item label="首次互动时间">{{
                formatDateTime(relationshipContext.firstInteractionAt)
              }}</el-descriptions-item>
              <el-descriptions-item label="关系持续天数"
                >{{
                  relationshipContext.relationshipAgeDays
                }}
                天</el-descriptions-item
              >
              <el-descriptions-item label="全局最后成功互动">{{
                formatDateTime(relationshipContext.globalLastCommittedAt)
              }}</el-descriptions-item>
              <el-descriptions-item label="当前角色最后成功互动">{{
                formatDateTime(relationshipContext.relationshipLastCommittedAt)
              }}</el-descriptions-item>
              <el-descriptions-item label="本次关系 Gap">{{
                formatDuration(relationshipContext.relationshipGapSeconds)
              }}</el-descriptions-item>
              <el-descriptions-item label="本次全局 Gap">{{
                formatDuration(relationshipContext.globalGapSeconds)
              }}</el-descriptions-item>
              <el-descriptions-item label="期望互动间隔">{{
                formatDuration(relationshipContext.expectedGapSeconds)
              }}</el-descriptions-item>
              <el-descriptions-item label="Normalized Gap">{{
                formatDecimal(relationshipContext.normalizedGap)
              }}</el-descriptions-item>
              <el-descriptions-item label="连续性分数">{{
                formatPercent(relationshipContext.continuityScore)
              }}</el-descriptions-item>
              <el-descriptions-item label="重新适应剩余回合">{{
                relationshipContext.reacclimationTurnsLeft
              }}</el-descriptions-item>
              <el-descriptions-item label="存储紧张度">{{
                formatDecimal(relationshipContext.storedTension)
              }}</el-descriptions-item>
              <el-descriptions-item label="有效紧张度">{{
                formatDecimal(relationshipContext.effectiveTension)
              }}</el-descriptions-item>
            </el-descriptions>
            <el-alert
              v-if="relationshipDiagnosticsList.length"
              title="诊断信息"
              type="warning"
              show-icon
              :closable="false"
            ...
            >
              <ul class="diagnostics-list">
                <li v-for="item in relationshipDiagnosticsList" :key="item">
                  {{ item }}
                </li>
              </ul>
            </el-alert>
          ...
          <el-empty
            v-else
            description="功能已启用，但当前没有可展示的关系时间状态"
            :image-size="42"
          />
        </el-tab-pane>
        <el-tab-pane label="Reunion" name="reunion">
          <el-alert
            v-if="!relationshipFeatureEnabled"
            title="久别重逢不可用"
            description="Relationship Time 后端功能开关未启用。"
            type="info"
            show-icon
            :closable="false"
          />
          <template v-else>
            <el-descriptions
              v-if="relationshipContext?.reunion"
              :column="2"
              border
              size="small"
              class="reunion-current"
            >
              <el-descriptions-item label="当前重逢类型">{{
                reunionKindLabel(relationshipContext.reunion.kind)
              }}</el-descriptions-item>
              <el-descriptions-item label="当前重逢等级">{{
                reunionLevelLabel(relationshipContext.reunion.level)
              }}</el-descriptions-item>
              <el-descriptions-item label="Episode 状态">{{
                reunionStateLabel(relationshipContext.reunion.state)
              }}</el-descriptions-item>
              <el-descriptions-item label="是否允许表达">{{
                relationshipContext.reunion.shouldExpress ? "是" : "否"
              }}</el-descriptions-item>
              <el-descriptions-item label="表达权所属 Interaction">{{
                relationshipContext.reunion.claimedByInteractionId || "—"
              }}</el-descriptions-item>
              <el-descriptions-item label="表达权到期时间">{{
                formatDateTime(relationshipContext.reunion.claimExpiresAt)
              }}</el-descriptions-item>
            </el-descriptions>
            <el-empty
              v-else
              description="当前没有活跃重逢事件"
              :image-size="42"
            />
            <div class="table-heading">最近重逢事件</div>
            <el-table
              v-if="reunionEpisodes.length"
              :data="reunionEpisodes"
              size="small"
              stripe
            >
              <el-table-column label="检测时间" min-width="160"
                ><template #default="{ row }">{{
                  formatDateTime(row.detectedAtUtc)
                }}</template></el-table-column
              >
              <el-table-column label="类型" min-width="160"
                ><template #default="{ row }">{{
                  reunionKindLabel(row.reunionKind)
                }}</template></el-table-column
              >
              <el-table-column label="等级" width="100"
                ><template #default="{ row }">{{
                  reunionLevelLabel(row.reunionLevel)
                }}</template></el-table-column
              >
              <el-table-column label="间隔" width="130"
                ><template #default="{ row }">{{
                  formatDuration(row.relationshipGapSeconds)
                }}</template></el-table-column
              >
              <el-table-column label="状态" width="100"
                ><template #default="{ row }"
                  ><el-tag size="small" :type="reunionStateTag(row.status)">{{
                    reunionStateLabel(row.status)
                  }}</el-tag></template
                ></el-table-column
              >
              <el-table-column
                prop="handledInteractionId"
                label="处理 Interaction"
                min-width="170"
                show-overflow-tooltip
              />
              <el-table-column
                prop="suppressionReason"
                label="抑制原因"
                min-width="140"
                show-overflow-tooltip
              />
        ...
        <el-tab-pane label="Prompt Contribution" name="prompt">
          <pre class="prompt-block">{{
            temporalDiagnostics.promptSections?.[0]?.content ||
            "本轮未注入时间上下文"
          }}</pre>
        </el-tab-pane>
        <el-tab-pane label="Commit Effects" name="effects">
          <pre
            v-if="temporalDiagnostics.commitEffects?.length"
            class="json-block"
            >{{ prettyJSON(temporalDiagnostics.commitEffects) }}</pre
          >
          <el-empty v-else description="当前没有时间提交效果" :image-size="42"
        /></el-tab-pane>
      </el-tabs>
      <el-empty
        v-else
        description="Temporal Runtime 诊断尚未加载"
        :image-size="48"
      />
    </el-card>
    ...
    <el-card class="debug-card" shadow="hover">
      ...
      <el-descriptions v-if="data.currentState" :column="3" border size="small">
        <el-descriptions-item label="当前状态">
          <el-tag :type="stateTagType(data.currentState.currentState)">{{
            stateLabel(data.currentState.currentState)
          }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="开始时间">{{
          data.currentState.stateStartedAt
        }}</el-descriptions-item>
        <el-descriptions-item label="结束时间">{{
          data.currentState.stateEndsAt || "—"
        }}</el-descriptions-item>
      </el-descriptions>
    ...
    <el-card class="debug-card" shadow="hover">
      ...
      <el-descriptions
        ...
        size="small"
      >
        <el-descriptions-item label="起床">{{
          data.todaySchedule.wakeTime?.slice(11, 16) ||
          data.todaySchedule.wakeTime
        }}</el-descriptions-item>
        <el-descriptions-item label="午饭">{{
          data.todaySchedule.lunchTime?.slice(11, 16) ||
          data.todaySchedule.lunchTime
        }}</el-descriptions-item>
        <el-descriptions-item label="晚饭">{{
          data.todaySchedule.dinnerTime?.slice(11, 16) ||
          data.todaySchedule.dinnerTime
        }}</el-descriptions-item>
        <el-descriptions-item label="午睡">{{
          data.todaySchedule.hasNap
            ? data.todaySchedule.napStartTime?.slice(11, 16) +
              "~" +
              data.todaySchedule.napEndTime?.slice(11, 16)
            : "无"
        }}</el-descriptions-item>
        <el-descriptions-item label="睡觉">{{
          data.todaySchedule.sleepTime?.slice(11, 16) ||
          data.todaySchedule.sleepTime
        }}</el-descriptions-item>
        <el-descriptions-item label="休息日">{{
          data.todaySchedule.isRestDay ? "是" : "否"
        }}</el-descriptions-item>
      </el-descriptions>
    ...
    <el-card class="debug-card" shadow="hover">
      <template #header>
        ...
        <el-button
          ...
          @click="triggerActiveMsg"
          :loading="triggeringActive"
          >手动触发执行器</el-button
        >
      </template>
      <el-table
        v-if="data.activeMessageTasks?.length"
        :data="data.activeMessageTasks"
        size="small"
        stripe
        max-height="300"
      >
    ...
    <el-card class="debug-card" shadow="hover">
      <template #header>
        ...
        <el-button
          size="small"
          style="float: right"
          @click="triggerDelayedReply"
          :loading="triggeringDelayed"
          >手动触发处理器</el-button
        >
      </template>
      <el-table
        v-if="data.delayedReplies?.length"
        :data="data.delayedReplies"
        size="small"
        stripe
        max-height="300"
      >
        ...
        >
          <template #default="{ row }"
            >{{ (row.userMessage || "").slice(0, 40)
            }}{{ (row.userMessage || "").length > 40 ? "..." : "" }}</template
          >
        </el-table-column>
        <el-table-column
          prop="expectedReplyAfter"
          label="预计回复"
          width="160"
        />
        <el-table-column prop="status" label="状态" width="90">
    ...
    <el-card class="debug-card" shadow="hover">
      ...
      <el-table
        v-if="data.recentRuleLogs?.length"
        :data="data.recentRuleLogs"
        size="small"
        stripe
        max-height="300"
      >
...
import {
  getTemporalDiagnostics,
  listReunionEpisodes,
  type CivilTimeSnapshot,
...
const relationshipFeatureEnabled = computed(
  () =>
    temporalDiagnostics.value?.core.featureFlags.relationshipTimeEnabled ===
    true,
);
...
const relationshipDiagnosticsList = computed(() =>
  Array.from(
    new Set([
...
async function loadAll() {
  ...
  try {
    const res = await get<any>("/api/companion/debug/overview", {
      characterId: injectedCharacterId?.value ?? undefined,
    });
...
async function loadTemporal() {
  ...
  if (results[0].status === "fulfilled")
    temporalDiagnostics.value = results[0].value as TemporalDiagnostics;
  else {
  ...
  if (characterId && results[1]?.status === "fulfilled") {
    const result = results[1].value as
      | ReunionEpisode[]
      | { items: ReunionEpisode[] };
...
async function regenerateAll() {
  ...
  try {
    await post(
      `/api/companion/debug/regenerate-all?characterId=${injectedCharacterId?.value ?? ""}`,
    );
...
async function triggerActiveMsg() {
  ...
  try {
    await post(
      `/api/companion/debug/process-active-messages?characterId=${injectedCharacterId?.value ?? ""}`,
    );
...
async function triggerDelayedReply() {
  try {
    await ElMessageBox.confirm("将立即处理所有到期的延迟回复，确定？", "确认", {
      type: "warning",
    });
  } catch {
    return;
  ...
  try {
    await post(
      `/api/companion/debug/process-delayed-replies?characterId=${injectedCharacterId?.value ?? ""}`,
    );
...
async function triggerDailyRegen() {
  ...
  try {
    await post(
      `/api/companion/debug/trigger-daily-regeneration?characterId=${injectedCharacterId?.value ?? ""}`,
    );
...
async function runTask(id: number) {
  try {
    await post(
      `/api/companion/active-message/tasks/${id}/run?characterId=${injectedCharacterId?.value ?? ""}`,
    );
...
async function cancelTask(id: number) {
  try {
    await post(
      `/api/companion/active-message/tasks/${id}/cancel?characterId=${injectedCharacterId?.value ?? ""}`,
    );
...
async function cancelDelayed(id: number) {
  try {
    await post(
      `/api/companion/delayed-replies/${id}/cancel?characterId=${injectedCharacterId?.value ?? ""}`,
    );
    ElMessage.success("已取消");
    await loadAll();
  } catch {
...
function stateLabel(s: string) {
  const m: Record<string, string> = {
    ...
    EATING_DINNER: "晚饭",
    BEFORE_SLEEP: "睡前",
    PREPARING_CLASS: "准备上课",
    IN_CLASS: "上课中",
    AFTER_CLASS: "下课",
    STUDYING: "学习",
    BUSY: "忙碌",
    PREPARING_WORK: "准备上班",
    COMMUTING_TO_WORK: "上班路上",
    WORKING: "工作中",
    LUNCH_BREAK: "午休",
    COMMUTING_HOME: "下班路上",
    AFTER_WORK: "下班后",
    ...
    IN_EXAM: "考试中",
    AFTER_EXAM: "考完",
    PART_TIME_PREPARE: "准备兼职",
    PART_TIME_WORKING: "兼职中",
    PART_TIME_AFTER: "兼职结束",
    WORKOUT_PREPARE: "准备健身",
    WORKING_OUT: "健身中",
    AFTER_WORKOUT: "健身结束",
    LIBRARY_STUDYING: "图书馆",
    LIBRARY_BREAK: "图书馆休息",
    SICK_RESTING: "生病",
    LOW_ENERGY: "低精力",
    OVERTIME: "加班",
...
function stateTagType(s: string) {
  ...
  if (
    ...
    return "warning";
  if (s.includes("WORK") || s === "OVERTIME" || s.includes("PART_TIME"))
    return "danger";
  if (s === "IDLE" || s.includes("AFTER_") || s.includes("BREAK"))
    return "success";
  return "info";
}
...
function unwrapRelationshipContext(
  value:
    | RelationshipTimeContext
    | SnapshotField<RelationshipTimeContext>
    | null
    | undefined,
) {
  if (!value || typeof value !== "object") return null;
  if ("version" in value && "characterId" in value)
    return value as RelationshipTimeContext;
...
function reunionKindLabel(value: string) {
  return (
    (
      {
        global_return: "全局回归",
        relationship_reconnect: "关系重连",
        reply_to_recent_proactive: "回应近期主动消息",
...
function reunionLevelLabel(value: string) {
  return (
    (
      {
        none: "无",
...
function reunionStateLabel(value: string) {
  return (
    (
      {
        pending: "待处理",
...
function reunionStateTag(value: string) {
  if (value === "handled") return "success";
  if (value === "claimed") return "warning";
  if (value === "suppressed" || value === "expired") return "info";
  return "primary";
}
...
@media (max-width: 700px) {
  ...
  :deep(.temporal-runtime-card .el-descriptions__body .el-descriptions__table) {
    display: block;
  }
  :deep(.temporal-runtime-card .el-descriptions__cell) {
    display: block;
    width: 100%;
  }
}
