<template>
  <div class="extension-page">
    <ExtensionPageHeader
      title="技能管理"
      description="管理 Amitia 内置能力与现有工具适配状态。禁用后会立即从模型工具列表中移除。"
    >
      <template #actions
      ...
      >
        ...
        <el-table-column label="兼容性" width="120">
          <template #default="{ row }">
            <el-tooltip :content="row.compatibilityReason || '与当前引擎兼容'">
              <el-tag :type="row.compatible ? 'success' : 'danger'">{{
        ...
        <el-table-column label="最近运行" width="130">
          <template #default="{ row }">
            <el-tag
              v-if="row.latestRun"
              :type="statusType(row.latestRun.status)"
              size="small"
              >{{ statusLabel(row.latestRun.status) }}</el-tag
            >
...
const filters = reactive<{
  ...
  source: string;
}>({ enabled: undefined, trigger: "", source: "" });
...
async function toggle(skill: SkillView) {
  if (skill.enabled) {
    await ElMessageBox.confirm(
      `禁用后，模型将不能再调用“${skill.name}”。历史记录和授权不会删除。`,
      "确认禁用技能",
