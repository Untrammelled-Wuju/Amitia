...
<template>
  <div class="extension-page" v-loading="loading">
    <ExtensionPageHeader :title="skill?.name || '技能详情'">
      <template v-if="skill" #meta>
        <div class="detail-heading-meta">
          ...
          <div class="tag-list">
            ...
            <el-tag :type="skill.compatible ? 'success' : 'danger'">{{
              skill.compatible ? "兼容" : "不兼容"
            }}</el-tag>
          </div>
        </div>
      </template>
      <template v-if="skill" #actions>
        <div class="header-actions">
          <el-button
            v-if="
              skill.source === 'workflow' || skill.source === 'instructions'
            "
            @click="
              router.push({
          ...
          >
          <el-button
            v-if="skill.source === 'workflow'"
            :loading="forking"
            @click="forkRevision"
            >创建新 Revision</el-button
          >
      ...
      <el-card shadow="never">
        ...
        <el-descriptions :column="3" border class="summary-descriptions">
          ...
          <el-descriptions-item label="超时"
            >{{ skill.timeoutMs }} ms</el-descriptions-item
          >
          <el-descriptions-item label="副作用">{{
            skill.hasSideEffects ? "有" : "无"
          }}</el-descriptions-item>
          <el-descriptions-item label="幂等">{{
            skill.idempotent ? "支持" : "不支持"
          }}</el-descriptions-item>
          <el-descriptions-item label="自动重试">{{
            skill.retryable ? "有限重试" : "不重试"
          }}</el-descriptions-item>
        </el-descriptions>
      </el-card>
      ...
      <el-tabs v-model="activeTab" class="detail-tabs">
        <el-tab-pane label="能力与协议" name="protocol">
          <div class="two-column-grid">
            <el-card shadow="never">
              ...
              <div v-if="skill.capabilities.length" class="capability-list">
                <div
                  v-for="name in skill.capabilities"
                  :key="name"
                  class="capability-item"
                >
                  <div>
                    <code>{{ name }}</code>
                    <p>{{ capabilityFor(name)?.description || "未知能力" }}</p>
                  </div>
                  <el-tag :type="riskType(capabilityFor(name)?.risk)">{{
                    riskLabel(capabilityFor(name)?.risk)
                  }}</el-tag>
                </div>
              </div>
              <el-empty
                v-else
                description="此技能不声明 Capability"
                :image-size="64"
              />
            </el-card>
            <el-card shadow="never">
              ...
              <div class="tag-list">
                <el-tag
                  ...
                  :key="trigger"
                  type="info"
                  >{{ triggerLabel(trigger) }}</el-tag
                >
              </div>
              <el-alert
                v-if="!skill.triggers.includes('manual')"
                title="该技能不支持手动执行"
                type="info"
                :closable="false"
                show-icon
          ...
          <div class="schema-grid">
            <el-card shadow="never"
              ><template #header>Manifest</template>
              <pre>{{ pretty(skill.manifest) }}</pre>
            </el-card>
            <el-card shadow="never"
              ><template #header>输入 Schema</template>
              <pre>{{ pretty(skill.inputSchema) }}</pre>
            </el-card>
            <el-card shadow="never"
              ><template #header>输出 Schema</template>
              <pre>{{ pretty(skill.outputSchema) }}</pre>
            </el-card>
          </div>
        </el-tab-pane>
            ...
            <el-form label-position="top" @submit.prevent>
              <el-form-item label="当前角色配置 JSON">
                <el-input
                  ...
                  :rows="14"
                  spellcheck="false"
                  aria-label="当前角色配置 JSON"
                />
                <div class="field-help">
                  配置会按技能的 Config Schema 校验。包含 Secret
                  的字段不会在页面或日志中明文显示。
                </div>
              </el-form-item>
              <div v-if="configError" class="field-error" role="alert">
                {{ configError }}
              </div>
              <div class="form-actions">
                <el-button :loading="resetting" @click="resetSkillConfig"
                  >恢复默认</el-button
                >
                <el-button
                  type="primary"
            ...
            <el-form label-position="top" @submit.prevent>
              <el-form-item label="输入 JSON">
                <el-input
                  ...
                  :rows="12"
                  spellcheck="false"
                  aria-label="技能测试输入 JSON"
                />
                <div class="field-help">
                  前端只检查 JSON 格式，最终字段和类型由后端 Schema Validator
                  校验。
                </div>
              </el-form-item>
              ...
              <div class="form-actions">
                ...
                <el-button
                  type="primary"
                  :loading="executing"
                  :disabled="!skill.triggers.includes('manual')"
                  @click="runTest"
                  >执行技能</el-button
                >
              </div>
          ...
          <el-card v-if="testResult" shadow="never" class="result-card">
            <template #header
              ><div class="result-header">
                <span>执行结果</span
                ><el-tag :type="statusType(testResult.status)">{{
                  statusLabel(testResult.status)
                }}</el-tag>
              </div></template
            >
            <el-descriptions :column="3" border>
              <el-descriptions-item label="runId" :span="2"
                ><code>{{ testResult.runId }}</code></el-descriptions-item
              >
              <el-descriptions-item label="耗时"
                >{{ testResult.durationMs }} ms</el-descriptions-item
              >
              <el-descriptions-item v-if="testResult.error" label="错误码">{{
              ...
              <el-descriptions-item
                ...
                :span="2"
                >{{
                  testResult.error.detail || testResult.error.message
                }}</el-descriptions-item
              >
            </el-descriptions>
            <pre v-if="testResult.output">{{ pretty(testResult.output) }}</pre>
            <pre v-if="testResult.sideEffects?.length">{{
              pretty(testResult.sideEffects)
            }}</pre>
          </el-card>
        </el-tab-pane>
        ...
        <el-tab-pane label="最近运行" name="runs">
          <el-card shadow="never" class="table-card">
            ...
            <div class="runs-footer">
              <el-button @click="router.push('/extensions/runs')"
                >查看全部执行记录</el-button
              >
            </div>
          </el-card>
            ...
            <el-table :data="skill.versions" empty-text="暂无历史版本" stripe>
              <el-table-column prop="version" label="版本" width="140"
                ><template #default="{ row }"
                  ...
                  ><el-tag
                    v-if="row.version === skill.version"
                    size="small"
                    type="success"
                    >当前</el-tag
                  ></template
              ...
              <el-table-column
                ...
                show-overflow-tooltip
              />
              <el-table-column prop="createdAt" label="安装时间" min-width="170"
                ><template #default="{ row }">{{
              ...
              <el-table-column label="操作" width="190" fixed="right"
                ><template #default="{ row }"
                  ><el-button link type="primary" @click="compareVersion(row)"
                    >比较</el-button
                  ><el-button
                    link
                    type="warning"
                    :disabled="row.version === skill.version"
                    :loading="rollingBack === row.version"
    ...
    <el-dialog
      ...
      :title="`版本比较：${compareTarget?.version || ''} ↔ ${skill?.version || ''}`"
      width="min(1080px, 94vw)"
      destroy-on-close
    >
      <div class="version-compare">
        <section>
          <h3>{{ compareTarget?.version }} Manifest</h3>
          <pre>{{ pretty(compareTarget?.manifest) }}</pre>
        </section>
        <section>
          <h3>{{ skill?.version }} Manifest</h3>
          <pre>{{ pretty(skill?.manifest) }}</pre>
        </section>
      </div>
      <template #footer
        ><el-button @click="compareVisible = false">关闭</el-button></template
      >
    </el-dialog>
...
async function load() {
  ...
  try {
    ...
    const [detail, catalog] = await Promise.all([
      fetchSkill(characterId.value, skillId.value),
      fetchCapabilities(),
    ]);
    skill.value = detail;
...
async function rollbackVersion(version: string) {
  if (!skill.value) return;
  await ElMessageBox.confirm(
    `将 ${skill.value.id} 回滚到 ${version}，现有授权不会自动恢复。`,
    "确认回滚",
...
async function runTest() {
  ...
  try {
    testResult.value = await executeSkill(skill.value.id, {
