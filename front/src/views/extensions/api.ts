...
export async function fetchSkills(
  characterId: string,
  filters: { enabled?: boolean; trigger?: SkillTrigger | ""; source?: string },
) {
  const response = await apiClient.get("/api/extensions/skills", {
    params: { characterId, ...filters },
  });
...
export async function fetchSkill(characterId: string, id: string) {
  const response = await apiClient.get(
    `/api/extensions/skills/${encodeURIComponent(id)}`,
    { params: { characterId } },
  );
...
export async function setSkillEnabled(
  ...
  await apiClient.post(
    `/api/extensions/skills/${encodeURIComponent(id)}/${enabled ? "enable" : "disable"}`,
    null,
    { params: { characterId } },
  );
}
...
export async function updatePermissions(
  ...
  await apiClient.put(
    `/api/extensions/skills/${encodeURIComponent(id)}/permissions`,
    {
      characterId,
      grants: grants.map(
        ({ capability, decision, scopeType, scopeId, expiresAt }) => ({
          capability,
          decision,
          scopeType,
          scopeId,
...
export async function updateConfig(
  ...
  await apiClient.put(
    `/api/extensions/skills/${encodeURIComponent(id)}/config`,
    config,
    { params: { characterId } },
  );
}
...
export async function resetConfig(id: string, characterId: string) {
  await apiClient.post(
    `/api/extensions/skills/${encodeURIComponent(id)}/config/reset`,
    null,
    { params: { characterId } },
  );
}
...
export async function executeSkill(
  ...
  const response = await apiClient.post(
    `/api/extensions/skills/${encodeURIComponent(id)}/execute`,
    payload,
  );
...
export async function fetchRuns(
  ...
  const response = await apiClient.get("/api/extensions/runs", {
    params: { characterId, ...filters },
  });
...
const pluginPath = (id: string) =>
  `/api/extensions/plugins/${encodeURIComponent(id)}`;

export async function fetchPlugins(page = 1, pageSize = 20) {
  const response = await apiClient.get("/api/extensions/plugins", {
...
export async function fetchPlugin(characterId: string, id: string) {
  const response = await apiClient.get(pluginPath(id), {
    params: { characterId },
  });
...
export async function setPluginEnabled(
  ...
  await apiClient.post(
    `${pluginPath(id)}/${enabled ? "enable" : "disable"}`,
    null,
    { params: { characterId } },
  );
}
...
export async function reloadPlugin(id: string) {
  await apiClient.post(`${pluginPath(id)}/reload`);
}
...
export async function updatePluginConfig(
  ...
  await apiClient.put(`${pluginPath(id)}/config`, config, {
    params: { characterId },
  });
}

export async function resetPluginConfig(id: string, characterId: string) {
  await apiClient.post(`${pluginPath(id)}/config/reset`, null, {
    params: { characterId },
  });
}

export async function updatePluginPermissions(
  id: string,
  characterId: string,
  grants: PermissionGrant[],
) {
  await apiClient.put(`${pluginPath(id)}/permissions`, {
    characterId,
    grants: grants.map(
      ({ capability, decision, scopeType, scopeId, expiresAt }) => ({
        capability,
...
export async function resetPluginCircuit(id: string) {
  await apiClient.post(`${pluginPath(id)}/circuit/reset`);
}

export async function fetchPluginState(id: string, characterId: string) {
  const response = await apiClient.get(`${pluginPath(id)}/state`, {
    params: { characterId },
  });
...
export async function fetchPluginSchedules(id: string, characterId: string) {
  const response = await apiClient.get(`${pluginPath(id)}/schedules`, {
    params: { characterId },
  });
  return response.data as PluginSchedule[];
}

export async function setPluginScheduleEnabled(
  id: string,
  scheduleId: string,
  enabled: boolean,
  characterId: string,
  ...
  await apiClient.post(
    `${pluginPath(id)}/schedules/${encodeURIComponent(scheduleId)}/${enabled ? "resume" : "pause"}`,
    null,
    { params: { characterId } },
  );
}

export async function fetchPluginEvents(
  id: string,
  characterId: string,
  status = "",
  page = 1,
  ...
  const response = await apiClient.get(`${pluginPath(id)}/events`, {
    params: { characterId, status, page, pageSize },
  });
  return response.data as PluginEventPage;
}

export async function retryPluginEvent(
  id: string,
  eventId: string,
  characterId: string,
) {
  await apiClient.post(
    `${pluginPath(id)}/events/${encodeURIComponent(eventId)}/retry`,
    null,
    { params: { characterId } },
  );
}
...
export async function executePluginAction(
  ...
  const response = await apiClient.post(
    `${pluginPath(id)}/surface/actions/${encodeURIComponent(actionId)}`,
    { characterId, input },
  );
  return response.data as SkillResult;
}

const workshopPath = (id = "") =>
  `/api/extensions/workshop/sessions${id ? `/${encodeURIComponent(id)}` : ""}`;
export async function fetchWorkshopSessions(
  characterId: string,
  page = 1,
  pageSize = 20,
) {
  const response = await apiClient.get(workshopPath(), {
    params: { characterId, page, pageSize },
  });
  return response.data as WorkshopPage;
}
export async function createWorkshopSession(
  ...
  return response.data as WorkshopSession;
}
export async function fetchWorkshopSession(id: string, characterId: string) {
  const response = await apiClient.get(workshopPath(id), {
    params: { characterId },
  });
  return response.data as WorkshopSessionDetail;
}
export async function archiveWorkshopSession(id: string, characterId: string) {
  await apiClient.post(`${workshopPath(id)}/archive`, null, {
    params: { characterId },
  });
}
export async function generateWorkshopDraft(
  id: string,
  characterId: string,
  payload: { requirement?: string; draft?: unknown } = {},
) {
  const response = await apiClient.post(
    `${workshopPath(id)}/generate`,
    payload,
    { params: { characterId } },
  );
  return response.data as WorkshopRevision;
}
export async function validateWorkshopDraft(
  ...
  const response = await apiClient.post(
    `${workshopPath(id)}/revisions/${revision}/validate`,
    null,
    { params: { characterId } },
  );
  return response.data as WorkshopValidation;
}
export async function confirmWorkshopPermissions(
  id: string,
  revision: number,
  characterId: string,
  payload: {
    ...
    capabilities: string[];
    confirmedHighRisk: string[];
    production: boolean;
  },
) {
  await apiClient.post(
    `${workshopPath(id)}/revisions/${revision}/permissions/confirm`,
    payload,
    { params: { characterId } },
  );
}
export async function testWorkshopDraft(
  id: string,
  revision: number,
  characterId: string,
  payload: {
    mode: string;
    testCases?: unknown[];
    controlledLiveConfirmed?: boolean;
  },
) {
  const response = await apiClient.post(
    `${workshopPath(id)}/revisions/${revision}/test`,
    payload,
    { params: { characterId } },
  );
  return response.data as WorkshopTestReport;
}
export async function installWorkshopDraft(
  id: string,
  revision: number,
  characterId: string,
) {
  const response = await apiClient.post(
    `${workshopPath(id)}/revisions/${revision}/install`,
    null,
    { params: { characterId } },
  );
...
export async function forkWorkflowSkill(id: string, characterId: string) {
  const response = await apiClient.post(
    `/api/extensions/skills/${encodeURIComponent(id)}/workshop/fork`,
    null,
    { params: { characterId } },
  );
  return response.data as WorkshopSessionDetail;
}
export async function rollbackWorkflowSkill(
  id: string,
  version: string,
  characterId: string,
) {
  const response = await apiClient.post(
    `/api/extensions/skills/${encodeURIComponent(id)}/versions/${encodeURIComponent(version)}/rollback`,
    null,
    { params: { characterId } },
  );
  return response.data as {
...
export async function generateWorkshopInstruction(
  requirement: string,
  characterId: string,
) {
  const response = await apiClient.post(
    "/api/extensions/workshop/instructions/generate",
    { requirement },
    { params: { characterId } },
  );
