  ...
  <main class="workflow-list-page" v-loading="loading">
    ...
    <section v-if="filteredWorkflows.length" class="workflow-grid">
      <article v-for="item in filteredWorkflows" :key="item.id" class="workflow-card" :class="{ selected: selectedIds.has(item.id) }">
        <div class="card-top">
          <el-checkbox :model-value="selectedIds.has(item.id)" :disabled="item.offline" @change="(v: boolean | string | number) => toggleSelected(item.id, Boolean(v))" />
          <div class="workflow-icon"><el-icon><Share /></el-icon></div>
          <div class="title-wrap">
            <h2>{{ item.name }}</h2>
            <p>{{ item.description || "未填写描述" }}</p>
          </div>
          <el-switch :model-value="item.enabled" :disabled="item.offline" @change="(v: boolean | string | number) => toggle(item, Boolean(v))" />
        </div>
        <div class="stats">
...
async function createWithAI() {
  try {
    ...
    await router.push({ path: `/extensions/workflows/${encodeURIComponent(created.id)}`, query: workflowTargetQuery(currentTarget.value) });
  } catch (e: any) {
...
async function createNew() {
  ...
  try {
    ...
    await router.push({ path: `/extensions/workflows/${encodeURIComponent(def.id)}`, query: workflowTargetQuery(currentTarget.value) });
  } catch (e: any) {
...
function edit(item: WorkflowDefinition) {
  ...
  router.push({ path: `/extensions/workflows/${encodeURIComponent(item.id)}`, query: workflowTargetQuery(currentTarget.value) });
}
...
async function runNow() {
  ...
  try {
    ...
    if (result.status === "waiting_confirmation") {
      ...
      await router.push({ path: `/extensions/workflows/${encodeURIComponent(item.id)}`, query: { ...workflowTargetQuery(currentTarget.value), runId: result.executionId } });
      return;
    }
