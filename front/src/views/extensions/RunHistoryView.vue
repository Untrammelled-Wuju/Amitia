...
<template>
  <div class="extension-page">
    <ExtensionPageHeader
      title="执行记录"
      description="按角色查看技能运行状态、触发来源、耗时与审计关联信息。"
    >
      <template #actions
    ...
    <el-card shadow="never" class="filter-card">
      <el-form :inline="true" label-position="top" @submit.prevent>
        ...
        <el-form-item label="渠道">
          <el-input
            v-model.trim="filters.channel"
            clearable
            placeholder="如 web、qq、wechat"
            @keyup.enter="search"
            @clear="search"
          />
        </el-form-item>
    ...
    <el-card shadow="never" class="table-card">
      ...
      >
        ...
        >
        <el-table-column prop="skillId" label="技能" min-width="230"
          ><template #default="{ row }"
            ><button class="run-link" type="button" @click.stop="openRun(row)">
      ...
      <template v-if="selectedRun">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="runId" :span="2"
            ><code>{{ selectedRun.runId }}</code></el-descriptions-item
          >
          <el-descriptions-item label="技能" :span="2"
            ><code>{{ selectedRun.skillId }}</code></el-descriptions-item
          >
          <el-descriptions-item label="状态"
            ><el-tag :type="statusType(selectedRun.status)">{{
              statusLabel(selectedRun.status)
            }}</el-tag></el-descriptions-item
          >
          <el-descriptions-item label="耗时"
            >{{ selectedRun.durationMs }} ms</el-descriptions-item
          >
          <el-descriptions-item label="触发">{{
            triggerLabel(selectedRun.trigger)
          }}</el-descriptions-item>
          <el-descriptions-item label="渠道">{{
            selectedRun.channel || "—"
          }}</el-descriptions-item>
          <el-descriptions-item label="角色"
            ><code>{{ selectedRun.characterId }}</code></el-descriptions-item
          >
          ...
          <el-descriptions-item label="traceId" :span="2"
            ><code>{{ selectedRun.traceId || "—" }}</code></el-descriptions-item
          >
          <el-descriptions-item v-if="selectedRun.errorCode" label="错误码">{{
            selectedRun.errorCode
          }}</el-descriptions-item>
          <el-descriptions-item
            v-if="selectedRun.errorDetail"
            label="错误详情"
            :span="selectedRun.errorCode ? 1 : 2"
            >{{ selectedRun.errorDetail }}</el-descriptions-item
          >
        </el-descriptions>
        <el-card shadow="never" class="summary-card"
          ><template #header>输入摘要</template>
          <pre>{{ selectedRun.inputSummary || "—" }}</pre>
        </el-card>
        <el-card shadow="never" class="summary-card"
          ><template #header>输出摘要</template>
          <pre>{{ selectedRun.outputSummary || "—" }}</pre>
        </el-card>
        <el-card
          v-if="selectedRun.sideEffects?.length"
          shadow="never"
          class="summary-card"
          ><template #header>副作用记录</template>
          <pre>{{ pretty(selectedRun.sideEffects) }}</pre>
        </el-card>
      </template>
    </el-drawer>
  </div>
...
const filters = reactive<{
  ...
  channel: string;
  range: string[];
}>({ skillId: "", status: "", trigger: "", channel: "", range: [] });
...
async function load() {
  ...
  try {
    ...
    if (!skills.value.length)
      skills.value = await fetchSkills(characterId.value, {});
    runs.value = await fetchRuns(characterId.value, {
