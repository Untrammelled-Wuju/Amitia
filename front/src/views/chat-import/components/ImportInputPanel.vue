...
你好呀！"
      ...
      <div class="input-options">
        <el-input
          :model-value="batchTitle"
          placeholder="标题（可选）"
          size="small"
          style="width: 200px"
          @update:model-value="$emit('update:batchTitle', $event)"
        />
        <div class="format-picker">
          <span class="fp-label">格式：</span>
          <el-radio-group
            :model-value="parseFormat"
            size="small"
            @update:model-value="$emit('update:parseFormat', $event)"
          >
      ...
      <el-collapse
        :model-value="showSpeakerOptions"
        style="border: none"
        @update:model-value="$emit('update:showSpeakerOptions', $event)"
      >
        <el-collapse-item title="自定义发言者名称映射" name="options">
          <div class="speaker-options">
            <div class="so-group">
              ...
              <el-input
                :model-value="userSpeakerInput"
                placeholder="例如：张三、我、自己"
                size="small"
                @update:model-value="$emit('update:userSpeakerInput', $event)"
              />
            </div>
            <div class="so-group">
              <span class="so-label">AI 发言者（逗号分隔）：</span>
              <el-input
                :model-value="assistantSpeakerInput"
                placeholder="例如：AI、李四、Bot"
                size="small"
                @update:model-value="
                  $emit('update:assistantSpeakerInput', $event)
                "
              />
            </div>
          </div>
      ...
      <div class="input-actions">
        ...
        <div class="default-role-picker">
          ...
          <el-radio-group
            :model-value="defaultRole"
            size="small"
            @update:model-value="$emit('update:defaultRole', $event)"
          >
