  ...
  <div class="psyche-detail">
    ...
    <template v-else>
      <div class="psyche-top-row">
        <div class="affect-badge" :class="affectClass">{{ psycheState.affectLabel || '未初始化' }}</div>
        <span class="collect-time">采样时间: {{ formatTime(psycheState.collectedAt) }}</span>
      </div>
        ...
        <div class="psyche-panel">
          ...
          <div class="emotion-bars">
            <div class="ebar-row">
              ...
              <div class="ebar-track"><div class="ebar-fill positive-fill" :style="{ width: pct(psycheState.emotion.positive) }"></div></div>
              <span class="ebar-val">{{ pct(psycheState.emotion.positive) }}</span>
            </div>
            <div class="ebar-row">
              <span class="ebar-label negative-label">消极</span>
              <div class="ebar-track"><div class="ebar-fill negative-fill" :style="{ width: pct(psycheState.emotion.negative) }"></div></div>
              <span class="ebar-val">{{ pct(psycheState.emotion.negative) }}</span>
            </div>
            <div class="ebar-row">
              <span class="ebar-label">唤醒度</span>
              <div class="ebar-track"><div class="ebar-fill arousal-fill" :style="{ width: pct(psycheState.emotion.arousal) }"></div></div>
              <span class="ebar-val">{{ pct(psycheState.emotion.arousal) }}</span>
            </div>
            <div class="ebar-row">
              <span class="ebar-label">支配度</span>
              <div class="ebar-track"><div class="ebar-fill dominance-fill" :style="{ width: pct(psycheState.emotion.dominance) }"></div></div>
              <span class="ebar-val">{{ pct(psycheState.emotion.dominance) }}</span>
            </div>
          </div>
        ...
        <div class="psyche-panel">
          ...
          <div class="mood-bars">
            <div class="ebar-row">
              ...
              <div class="ebar-track"><div class="ebar-fill valence-fill" :style="{ width: pct(psycheState.mood.valence) }"></div></div>
              <span class="ebar-val">{{ pct(psycheState.mood.valence) }}</span>
            </div>
            <div class="ebar-row">
              <span class="ebar-label">张力</span>
              <div class="ebar-track"><div class="ebar-fill tension-fill" :style="{ width: pct(psycheState.mood.tension) }"></div></div>
          ...
          <div class="stress-energy">
            <div class="se-item">
              <div class="se-header">
                ...
                <span class="se-val" :class="stressLevel">{{ pct(psycheState.stress) }}</span>
              </div>
              <div class="se-ring-track">
                <div class="se-ring-fill" :class="stressLevel" :style="{ width: pct(psycheState.stress) }"></div>
              </div>
            </div>
            <div class="se-item">
              <div class="se-header">
                <span class="se-label">精力</span>
                <span class="se-val" :class="energyLevel">{{ pct(psycheState.energy) }}</span>
              </div>
              <div class="se-ring-track">
                <div class="se-ring-fill energy-fill" :class="energyLevel" :style="{ width: pct(psycheState.energy) }"></div>
              </div>
            </div>
          </div>
        ...
        <div class="needs-grid">
          <div class="need-item" v-for="[k, v] in needsEntries" :key="k">
            ...
            <div class="need-bar-track">
              <div class="need-bar-fill" :style="{ width: Math.min(v * 100, 100).toFixed(0) + '%', backgroundColor: needColor(v) }"></div>
            </div>
          </div>
        </div>
        ...
        <div class="rel-grid">
          <div class="rel-item" v-for="rel in relEntries" :key="rel.key">
            ...
            <div class="rel-bar-track">
              <div class="rel-bar-fill" :style="{ width: pct(rel.value), backgroundColor: relColor(rel.value) }"></div>
            </div>
          </div>
        </div>
      ...
      <div class="psyche-panel beliefs-panel" v-if="psycheState.beliefs?.length">
        <div class="panel-header">
          <span class="panel-icon">🧠</span>
          <span>活跃信念 ({{ psycheState.beliefs.length }})</span>
        </div>
        <div class="beliefs-list">
          <div class="belief-card" v-for="b in psycheState.beliefs" :key="b.key">
            <div class="belief-top">
              <span class="belief-key">{{ b.key }}</span>
              <el-tag :type="b.conflicted ? 'danger' : 'success'" size="small" effect="plain">
                {{ b.conflicted ? '冲突' : '一致' }}
              </el-tag>
            </div>
            ...
            <div class="belief-conf-bar">
              <div class="bcb-track">
                <div class="bcb-fill" :class="{ 'bcb-low': b.confidence < 0.4, 'bcb-mid': b.confidence >= 0.4 && b.confidence < 0.7, 'bcb-high': b.confidence >= 0.7 }" :style="{ width: pct(b.confidence) }"></div>
              </div>
              <span class="bcb-val">{{ (b.confidence * 100).toFixed(0) }}%</span>
            </div>
          </div>
        </div>
...
const emotionDesc = computed(() => {
  ...
  else if (n > 0.65 && p < 0.35) desc = '当前情绪偏向消极，整体低落感较为明显。'
...
const needsDesc = computed(() => {
  ...
  const high = entries.filter(([, v]) => v > 0.65).map(([k]) => formatNeedLabel(k))
  const low = entries.filter(([, v]) => v < 0.35).map(([k]) => formatNeedLabel(k))
  let desc = ''
  if (high.length) desc = `当前亟需满足的需求：${high.join('、')}。`
  if (low.length) desc += ` 已充分满足的需求：${low.join('、')}。`
  if (!high.length && !low.length) desc = '各需求处于均衡状态，无明显缺口或溢出。'
  return desc
})
...
function formatNeedLabel(k: string): string {
  const map: Record<string, string> = {
    reassurance: '安心', connection: '连接', autonomy: '自主',
    clarity: '清晰', rest: '休息', expression: '表达', novelty: '新鲜',
  }
  return map[k] || k
}
