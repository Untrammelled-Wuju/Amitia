...
export function useProactiveRules() {
  const injectedCharacterId = inject<Ref<string>>(
    "currentCharacterId",
    ref(""),
  );
  ...
  function openCreateDialog() {
    ...
    resetForm();
    if (injectedCharacterId?.value)
      form.value.characterId = injectedCharacterId.value;
  ...
  async function toggleRule(row: ProactiveRule, val: boolean) {
    try {
      const result: any = await request.post(
        `/api/proactive/rules/${row.id}/toggle`,
      );
  ...
  async function fetchConversations() {
    try {
      const res: any = await request.get("/api/chats/conversations", {
        params: { pageSize: 100 },
      });
  ...
  async function fetchCharacters() {
    try {
      ...
      characters.value = Array.isArray(res)
        ? res
        : res?.items || res?.data || [];
    } catch {}
  }
