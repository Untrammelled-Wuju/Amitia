    ...
    <el-dialog v-model="conflictVisible" title="记忆冲突检测" width="550px" destroy-on-close :close-on-click-modal="false">
      <div v-if="conflictList.length === 0">
        <el-form label-position="top">
          ...
          <el-form-item label="新记忆内容"><el-input v-model="conflictNewContent" type="textarea" :rows="3" /></el-form-item>
          <el-form-item>
            <el-button type="primary" @click="checkConflict" :loading="checking">检查冲突</el-button>
          </el-form-item>
        </el-form>
      </div>
      <div v-else>
        <h4 style="margin-bottom:8px">发现 {{ conflictList.length }} 条潜在冲突</h4>
        <div v-for="c in conflictList" :key="c.id" class="conflict-item">
          <div class="ci-header"><el-tag size="small">{{ typeLabel(c.memoryType) }}</el-tag> 重要度: {{ c.importance }}/10</div>
          <div class="ci-key">{{ c.key }}</div>
          <div class="ci-value">{{ c.value }}</div>
          <div class="ci-reason"><el-tag size="small" type="danger">冲突原因</el-tag> {{ c.reason }}</div>
        </div>
        ...
        <el-button type="primary" @click="resolveConflict" :loading="resolving" style="margin-top:12px">解决冲突</el-button>
      </div>
    </el-dialog>
</template>
...
const emit = defineEmits<{ "update:modelValue": [value: boolean]; "conflict-resolved": [] }>()
...
<style scoped>
.conflict-item { padding: 10px; border: 1px solid var(--ac-color-border-light); border-radius: var(--ac-radius-sm); margin-bottom: 8px; }
.ci-header { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; }
.ci-key { font-weight: 600; font-size: var(--ac-font-size-sm); }
.ci-value { font-size: var(--ac-font-size-sm); color: var(--ac-color-text-secondary); }
.ci-reason { margin-top: 6px; font-size: var(--ac-font-size-xs); color: var(--ac-color-danger); }
</style>