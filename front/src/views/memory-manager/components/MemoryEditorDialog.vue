...
<template>
  ...
  >
    <el-form :model="form" label-position="top">
      ...
      <el-form-item label="使用权限">
        <div class="permission-switches">
          <el-checkbox v-model="form.allowContextUse"
            >允许用于上下文理解</el-checkbox
          >
          <el-checkbox v-model="form.allowProactiveMention"
            >允许主动提及</el-checkbox
          >
          <el-checkbox v-model="form.requiresConfirmation"
            >使用前需要确认</el-checkbox
          >
        </div>
      </el-form-item>
    ...
    <template #footer>
      ...
      <el-button type="primary" @click="saveMem" :loading="saving"
        >保存</el-button
      >
    </template>
  </el-dialog>
...
function initCreate() {
  ...
  form.importance = 5;
  form.characterId = props.characterId;
  form.source = "manual";
...
function initEdit(row: any) {
  ...
  form.sensitivity =
    ...
    "normal";
  form.allowContextUse = readFlag(
    row,
    ["allowContextUse", "allow_context_use"],
    true,
  );
  form.allowProactiveMention = readFlag(
    row,
    ["allowProactiveMention", "allow_proactive_mention"],
    false,
  );
  form.requiresConfirmation = readFlag(
    row,
