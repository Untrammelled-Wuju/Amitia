  ...
  <div class="mem-page">
    ...
    <div class="pipeline-bar" v-if="pipelineStatus">
      ...
      <template v-for="l in pipelineStatus.layers" :key="l.layer">
        <el-tooltip
          :content="l.name + ': ' + l.status + ' (' + l.durationMs + 'ms)'"
          placement="top"
        >
          <span
            class="pl-dot"
      ...
      <span class="pl-time" v-if="pipelineStatus.endedAt">{{
        fmtDate(pipelineStatus.endedAt)
      }}</span>
    </div>
    ...
    <div class="mem-toolbar">
      ...
      <router-link to="/graph"
        ><el-button size="small" type="info" plain>图谱</el-button></router-link
      >
    </div>
    ...
    <div class="global-search-bar">
      ...
      <el-button
        size="small"
        @click="showGlobalResults = !showGlobalResults"
        v-if="globalSearched"
      >
        {{
    ...
    <div v-if="showGlobalResults && globalSearched" class="global-results">
      <div v-if="globalResults.memories.length" class="gr-section">
        <h4>
          <el-tooltip content="按角色独立，切换角色后数据不同" placement="top"
            ><span class="gr-label">结构化记忆</span></el-tooltip
          >
        ...
        <div v-for="m in globalResults.memories" :key="m.id" class="gr-item">
          ...
          <span class="gr-score" v-if="m.score"
            >({{ (m.score * 100).toFixed(0) }}%)</span
          >
        </div>
      </div>
      <div v-if="globalResults.profiles.length" class="gr-section">
        <h4>
          <el-tooltip
            content="按用户共享，所有角色共用同一份画像"
            placement="top"
            ><span class="gr-label">用户画像</span></el-tooltip
          >
          ({{ globalResults.profiles.length }})
        </h4>
        <div v-for="p in globalResults.profiles" :key="p.id" class="gr-item">
          {{ p.attributeName }}: {{ p.attributeValue }}
        </div>
      </div>
      <div v-if="globalResults.episodics.length" class="gr-section">
        <h4>
          <el-tooltip content="按用户共享，跨角色的对话日记" placement="top"
            ><span class="gr-label">情景记忆</span></el-tooltip
          >
          ({{ globalResults.episodics.length }})
        </h4>
        <div v-for="e in globalResults.episodics" :key="e.id" class="gr-item">
          {{ e.title }}
        </div>
      </div>
      <div v-if="globalResults.worldBooks.length" class="gr-section">
        <h4>
          <el-tooltip content="全局共享，所有角色通用知识规则" placement="top"
            ><span class="gr-label">世界书</span></el-tooltip
          >
          ({{ globalResults.worldBooks.length }})
        </h4>
        <div v-for="w in globalResults.worldBooks" :key="w.id" class="gr-item">
          {{ w.matchPattern }}
        </div>
      </div>
      <el-empty
        v-if="globalResultCount === 0"
        description="未找到相关结果"
        :image-size="40"
      />
    </div>
    ...
    <el-tabs v-model="activeTab" class="mem-tabs">
        ...
        >
          <template #title>
            ...
            <el-button
              ...
              >{{ showCandidates ? "收起" : "查看" }}</el-button
            >
          </template>
        </el-alert>
      ...
      <el-tab-pane label="检索分析" name="analysis">
        <RetrievalAnalysisPanel
          :retrieval-stats="retrievalStats"
          :retrieval-logs="retrievalLogs"
          :halflife-episodic="halflifeEpisodic"
          :halflife-profile="halflifeProfile"
          :halflife-fact="halflifeFact"
          :halflife-worldbook="halflifeWorldbook"
        />
      </el-tab-pane>
    </el-tabs>
    ...
    <CandidateEditorDialog
      v-model="editCandidateVisible"
      @candidate-updated="loadCandidates"
    />
    <ConflictResolverDialog
      v-model="conflictVisible"
      @conflict-resolved="fetchList"
    />
...
const {
  ...
  fetchList,
  () => characterFilter.value || injectedCharacterId?.value,
);
const {
  searchDialogVisible,
  searchQuery,
