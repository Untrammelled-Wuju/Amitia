    ...
    <div v-else>
      <div class="mi-section" v-if="memories.length">
        ...
        <div v-for="m in memories" :key="m.id" class="mi-item">
          <el-tag
            size="small"
            :type="m.matchType === 'vector' ? 'success' : 'info'"
            >{{ m.matchType }}</el-tag
          >
      ...
      <div class="mi-section" v-if="profiles.length">
        ...
        <div v-for="p in profiles" :key="p.id" class="mi-profile-card">
          ...
          <el-tag
            :type="p.confidence >= 80 ? 'success' : 'warning'"
            size="small"
            >{{ p.confidence }}%</el-tag
          >
        </div>
      ...
      <div class="mi-section">
        ...
        <div class="mi-compress">
          <span
            >已压缩 {{ compression.compressedRounds || 0 }} /
            {{ compression.totalRounds || 0 }} 轮</span
          >
          <span v-if="compression.lastCompressedAt"
            >上次: {{ compression.lastCompressedAt }}</span
          >
        </div>
      </div>
      <div class="mi-section">
      ...
      <div class="mi-empty" v-if="!memories.length && !profiles.length">
        暂无记忆注入数据
      </div>
    </div>
  </div>
...
onMounted(async () => {
  ...
  try {
    if (props.convId) {
      try {
        const compR: any = await get(
          "/api/chats/" + props.convId + "/compression-status",
        );
        compression.value = compR || {};
      } catch {}
      try {
        const pipeR: any = await get("/api/memory/pipeline/status");
        pipeline.value = pipeR;
      } catch {}
      try {
        const profR: any = await get("/api/profiles", { pageSize: 5 });
        profiles.value = profR?.items || [];
      } catch {}
    }
  } catch {}
  loading.value = false;
