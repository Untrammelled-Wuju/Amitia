...
<template>
  <div class="page">
    ...
    >
      <template #title>
        请先在
        <router-link to="/settings/model">模型配置</router-link> 中设置语音识别
        API Key
      </template>
    </el-alert>
    ...
    >
      <template #title>
        录音文件识别需要音频的公网URL。本地开发请使用 ngrok
        暴露本地服务，或上传音频到云存储后填入URL。
      </template>
    </el-alert>
    ...
    <el-card>
      ...
      <div v-if="apiKeyConfigured">
        <span style="color: var(--el-color-success); font-size: 13px"
          >API Key 已配置 (从模型配置中读取)</span
        >
      </div>
      <div v-else>
        <span style="color: var(--el-color-danger); font-size: 13px"
          >未配置 API Key</span
        >
      </div>
    </el-card>
      ...
      <el-form label-position="top">
        <el-form-item label="上传音频文件">
          ...
          >
            ...
            <template #tip
              ><div class="form-hint">
                支持 mp3/wav/ogg/m4a/aac/pcm，最大60分钟
              </div></template
            >
          </el-upload>
        </el-form-item>
        ...
        <el-form-item label="音频公网URL">
          <el-input
            v-model="audioUrl"
            placeholder="输入音频的公网URL（或先上传文件获取本地URL后再用ngrok暴露）"
          />
          <div class="form-hint">
            火山引擎服务器需要能访问到此URL。本地文件先上传后复制地址填入
          </div>
        </el-form-item>
        <el-form-item>
          <el-button
            ...
            @click="submitTask"
            :loading="submitting"
            :disabled="!audioUrl.trim() || !apiKeyConfigured"
            >提交识别</el-button
          >
          <el-button @click="stopPolling" v-if="pollTimer" :loading="false"
            >停止轮询</el-button
          >
        </el-form-item>
      </el-form>
...
async function submitTask() {
  ...
  if (!apiKeyConfigured.value) {
    ElMessage.warning("请先在模型配置中设置语音识别API Key");
    return;
  }
