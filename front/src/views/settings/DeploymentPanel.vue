        ...
        <el-form label-position="top" :model="formData" :rules="rules" ref="formRef">
          <el-form-item label="服务器地址" prop="serverURL">
            <el-input
              ...
              clearable
            />
            <div class="form-tip">输入你部署的 Core 服务器完整地址，包含协议和端口</div>
          </el-form-item>
        </el-form>
      </div>
    ...
    <el-card shadow="never" class="section-card">
      ...
      <el-descriptions :column="2" border size="small">
        <el-descriptions-item label="当前模式">
          <el-tag :type="currentMode === 'local' ? 'success' : 'warning'" size="small">
            {{ currentMode === 'local' ? '本地模式' : '云端模式' }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="API 地址">{{ currentApiURL || '—' }}</el-descriptions-item>
...
import { getDeploymentConfig, saveDeploymentConfig, getApiBaseURL } from "../../runtime/runtime-adapter"
import type { DeploymentModeConfig, RuntimeStatus } from "../../runtime/runtime-types"
  ...
  try {
    const config: DeploymentModeConfig =
      formMode.value === "cloud"
        ? { mode: "cloud", serverURL: formData.serverURL.trim().replace(/\/+$/, "") }
        : { mode: "local" }
