<template>
  <section
    ...
    class="section-card relationship-time-section"
    aria-labelledby="relationship-time-title"
  >
    <div class="section-heading">
      <div>
        <div id="relationship-time-title" class="section-title">关系时间</div>
        <div class="section-subtitle">
          让角色理解相识时长、互动节奏与久别重逢，同时保持关系状态连续。
        </div>
      </div>
      <el-tag v-if="availability === 'available'" type="success" effect="plain"
        >服务可用</el-tag
      >
      <el-tag v-else-if="availability === 'error'" type="danger" effect="plain"
        >加载失败</el-tag
      >
      ...
      <el-form label-position="top" :model="settings" class="settings-grid">
        <el-form-item label="关系时间感知">
          ...
          <div class="field-help">
            关闭后不向角色注入相识时长、互动节奏或连续性信息。
          </div>
        </el-form-item>
        <el-form-item label="久别重逢">
          <el-switch
            v-model="settings.reunionEnabled"
            :disabled="!settings.enabled"
            active-text="开启"
            inactive-text="关闭"
          />
          <div class="field-help">
            只在符合节奏偏差与重逢规则时表达，不会因自然离线降低关系值。
          </div>
        </el-form-item>
        <el-form-item label="表达强度">
          ...
          <div class="field-help">
            控制时间感受的表达程度，不改变关系状态本身。
          </div>
        </el-form-item>
        <el-form-item label="最多重逢表达句数">
          <el-input-number
            v-model="settings.maxMentionSentences"
            :min="0"
            :max="2"
            :disabled="!settings.enabled || !settings.reunionEnabled"
          />
          <div class="field-help">
        ...
        <el-form-item label="允许提及重逢">
          <el-switch
            v-model="settings.allowReunionMention"
            :disabled="!settings.enabled || !settings.reunionEnabled"
          />
        </el-form-item>
      </el-form>
      ...
      <div class="status-panel" aria-live="polite">
        <div class="status-heading">
          <div>
            ...
            <div class="field-help">
              状态来自已成功提交的互动，不使用仅被观察到但失败的请求。
            </div>
          </div>
          <el-button :loading="stateLoading" @click="loadState"
            >刷新状态</el-button
          >
        </div>
        <el-alert
        ...
        <el-descriptions v-else-if="state" :column="2" border size="small">
          <el-descriptions-item label="首次互动">{{
            formatTime(state.firstInteractionAt)
          }}</el-descriptions-item>
          <el-descriptions-item label="关系持续"
            >{{ state.relationshipAgeDays }} 天</el-descriptions-item
          >
          <el-descriptions-item label="最近成功互动">{{
            formatTime(state.lastSuccessfulExchangeAt)
          }}</el-descriptions-item>
          <el-descriptions-item label="期望互动间隔">{{
            formatDuration(state.expectedGapSeconds)
          }}</el-descriptions-item>
          <el-descriptions-item label="连续性">{{
            formatScore(state.continuityScore)
          }}</el-descriptions-item>
          <el-descriptions-item label="重新适应剩余回合">{{
            state.reacclimationTurnsLeft
          }}</el-descriptions-item>
        </el-descriptions>
        <el-empty
          v-else
          description="尚未产生可展示的关系时间状态"
          :image-size="48"
        />
      </div>
...
import {
  getRelationshipTimeSettings,
  getRelationshipTimeState,
  getTemporalDiagnostics,
  updateRelationshipTimeSettings,
  type RelationshipTimeContext,
  type RelationshipTimeSettings,
} from "@/api/temporal";
...
const availability = ref<"checking" | "available" | "unavailable" | "error">(
  "checking",
);
const loadError = ref("");
...
async function save() {
  ...
  try {
    ...
    Object.assign(
      settings,
      await updateRelationshipTimeSettings(props.characterId, settings),
    );
