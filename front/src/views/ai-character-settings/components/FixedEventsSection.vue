...
<template>
  <div class="section-card">
    ...
    <div v-if="courses.length === 0" class="gender-hint" style="padding: 8px 0">
      暂无固定日程，点击上方按钮添加课程或会议
    </div>
    <div v-for="c in courses" :key="c.id" class="course-item">
      <div class="course-info">
        ...
        <span class="course-days" v-if="c.repeatDays">{{
          dayLabels(c.repeatDays)
        }}</span>
        <span class="course-reply"
          >回复：{{ REPLY_MODE_LABELS[c.replyMode] || c.replyMode }}</span
        >
      </div>
      <div class="course-actions">
        <el-switch
          v-model="c.enabled"
          size="small"
          @change="(v: any) => toggleCourse(c.id, v)"
        />
      ...
      <div class="course-form">
        ...
        <div class="form-row">
          <div class="form-item" style="flex: 1">
            ...
            <el-input-number
              v-model="courseForm.prepareMinMinutes"
              :min="5"
              :max="60"
              size="default"
              style="width: 100%"
          ...
          <div class="form-item" style="flex: 1">
            ...
            <el-input-number
              v-model="courseForm.prepareMaxMinutes"
              :min="5"
              :max="90"
              size="default"
              style="width: 100%"
        ...
        <div class="form-item">
          <label>回复模式</label>
          <el-select
            v-model="courseForm.replyMode"
            size="default"
            style="width: 100%"
          >
            <el-option
              v-for="opt in REPLY_MODE_OPTIONS"
              :key="opt.value"
              :label="opt.label"
              :value="opt.value"
            />
...
import {
  ...
  REPLY_MODE_OPTIONS,
  type FixedEvent,
} from "../../../composables/useFixedEvents";

const props = defineProps<{
  characterId: string;
}>();

const { getFixedEvents, createFixedEvent, updateFixedEvent, deleteFixedEvent } =
  useFixedEvents();
...
async function saveCourse() {
  ...
  try {
    const input = {
      ...
      repeatDays:
        courseForm.repeatDays.length > 0
          ? courseForm.repeatDays.join(",")
          : null,
