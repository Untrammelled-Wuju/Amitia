        ...
        <el-tabs v-model="inspectorTab" stretch @tab-change="onInspectorTabChanged">
          <el-tab-pane label="属性" name="properties">
            <div v-if="selectedNode" class="inspector-content">
              ...
              <label v-else-if="needsTarget(selectedNode.type)">目标 ID<el-input v-model="selectedNode.targetId" placeholder="Tool / MCP / Task ID" @change="markDirty" /></label>
                ...
                <template v-if="isEventTrigger(trigger.type)">
                  <label>触发方式
                    <el-select :model-value="eventTriggerPreset(trigger)" @change="applyEventTriggerPreset(trigger, $event)">
                      <el-option label="高级事件" value="advanced" />
                  ...
                  <template v-if="eventTriggerPreset(trigger) === 'voice_wake'">
                    ...
                    <p class="panel-tip" v-if="triggerWakeConfigs.length === 0">目标设备没有可用的 Wake Config，需先创建并启用唤醒配置。</p>
                  </template>
                ...
                <div>
                  ...
                  <div v-if="attemptsForNode(step.nodeId).length" class="attempt-list">
                    <span v-for="attempt in attemptsForNode(step.nodeId)" :key="`${attempt.generation}-${attempt.attempt}`" :class="`attempt-${attempt.status}`">
                      G{{ attempt.generation }} / #{{ attempt.attempt }} · {{ attempt.status }}<template v-if="attempt.nextBackoffMs"> · {{ attempt.nextBackoffMs }}ms 后重试</template>
                    </span>
                  </div>
                </div>
              </article>
...
const runs = ref<WorkflowRun[]>([]); const currentRun = ref<WorkflowRun | null>(null); const stepRuns = ref<WorkflowStepRun[]>([]); const stepAttempts = ref<WorkflowStepAttempt[]>([]); const checkpoints = ref<WorkflowCheckpoint[]>([]); let pollTimer: number | undefined;
...
function onExecutionTargetChanged() { markDirty(); void refreshNestedCandidates(); }
...
function addNode(type: string, position?: {x:number;y:number}) {
  ...
  const node: WorkflowNode = { id: `${type}-${Date.now().toString(36)}-${index+1}`, type, label: paletteByType(type)?.label || type, position: position || { x: 120 + (index%4)*230, y: 120 + Math.floor(index/4)*150 }, step: { input: {}, onError: { mode: "fail" } }, runtime: { runtimeType: defaultRuntimeType(type), metadata: {} }, executionTarget: { placement: isCloudWorkflow ? "cloud" : "local", offlinePolicy: "fail" } };
...
function selectNode(id: string) { selectedNodeId.value=id; selectedEdgeId.value=""; inspectorTab.value="properties"; syncNodeEditors(); void refreshNestedCandidates(); }
...
function onNodeTypeChanged() { const n=selectedNode.value; if(!n)return; const rt=defaultRuntimeType(n.type); if(rt) ensureRuntime(n).runtimeType=rt; ensureExecutionTarget(n); markDirty(); void refreshNestedCandidates(); }
...
async function startRun(){if(dirty.value){await save();if(dirty.value)return;}running.value=true;try{const res=await runWorkflow(workflow.id,{},false,workflowTarget);currentRun.value={executionId:res.executionId,workflowId:workflow.id,status:res.status||"running"};inspectorTab.value="runs";if(workflowTarget.location==="device"){ElMessage.success(`已提交到设备执行：${res.executionId}`);return;}startPolling(res.executionId);}finally{running.value=false;}}
function applyRunDetail(data: Awaited<ReturnType<typeof getWorkflowRun>>){currentRun.value=data.run;stepRuns.value=data.stepRuns||[];stepAttempts.value=data.attempts||[];checkpoints.value=data.checkpoints||[];}
function startPolling(runId:string){if(pollTimer)clearInterval(pollTimer);const tick=async()=>{try{const data=await getWorkflowRun(runId,workflowTarget);applyRunDetail(data);if(["succeeded","failed","cancelled","compensated"].includes(data.run.status)){if(pollTimer)clearInterval(pollTimer);pollTimer=undefined;await refreshObservability();}}catch{/* run can take a moment to be persisted */}};void tick();pollTimer=window.setInterval(tick,800);}
...
async function refreshRuns(){if(workflowTarget.location==="device"){runs.value=[];return;}const data=await listWorkflowRuns(workflow.id,40,workflowTarget);runs.value=data.items||[];}
async function refreshObservability(){if(workflowTarget.location==="device"){runs.value=[];executionStats.value=null;return;}await Promise.all([refreshRuns(), getWorkflowStats(workflow.id,workflowTarget).then(v=>executionStats.value=v).catch(()=>executionStats.value=null)]);}
...
async function openRun(runId:string){const data=await getWorkflowRun(runId,workflowTarget);applyRunDetail(data);if(!["succeeded","failed","cancelled","compensated"].includes(data.run.status))startPolling(runId);}
...
watch(selectedNodeId,()=>{syncNodeEditors();mappingTargetPath.value="";mappingSourceRef.value="";void refreshNestedCandidates();});
...
onMounted(async()=>{
  const [loaded,items,ownedWorkflows,devices,capabilities,appCatalog,wakeConfigs]=await Promise.all([
    ...
    workflowTarget.location === "device" ? Promise.resolve([] as WorkflowCatalogItem[]) : getWorkflowCatalog(workflowTarget).catch(()=>[] as WorkflowCatalogItem[]),
