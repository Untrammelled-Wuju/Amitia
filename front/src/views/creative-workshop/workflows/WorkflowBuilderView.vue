...
import {
  cancelWorkflowRun, createWorkflowRevision, editWorkflowWithAI, explainWorkflowWithAI, getWorkflow, getWorkflowAnalysis, getWorkflowCatalog, getWorkflowRun, getWorkflowStats, listWorkflowRevisions, listWorkflowRuns, listWorkflows, pauseWorkflowRun, recoverWorkflowRun, repairWorkflowWithAI, rerunWorkflowRun, resumeWorkflowRun, rollbackWorkflowRevision,
  runWorkflow, updateWorkflow, validateWorkflow,
  type WorkflowAIProposal, type WorkflowCatalogItem, type WorkflowCheckpoint, type WorkflowDefinition, type WorkflowEdge, type WorkflowExecutionStats, type WorkflowNode, type WorkflowRevisionSummary, type WorkflowRun, type WorkflowSafetyAnalysis, type WorkflowStepAttempt, type WorkflowStepRun, type WorkflowTrigger,
} from "@/api/workflow";
...
function startPolling(runId:string){if(pollTimer)clearInterval(pollTimer);const tick=async()=>{try{const data=await getWorkflowRun(runId);applyRunDetail(data);if(["succeeded","failed","cancelled","compensated"].includes(data.run.status)){if(pollTimer)clearInterval(pollTimer);pollTimer=undefined;await refreshObservability();}}catch{/* run can take a moment to be persisted */}};void tick();pollTimer=window.setInterval(tick,800);}
...
async function refreshObservability(){await Promise.all([refreshRuns(), getWorkflowStats(workflow.id).then(v=>executionStats.value=v).catch(()=>executionStats.value=null)]);}
async function refreshSafety(){try{safetyAnalysis.value=await getWorkflowAnalysis(workflow.id);}catch{safetyAnalysis.value=null;}}
async function openRun(runId:string){const data=await getWorkflowRun(runId);applyRunDetail(data);if(!["succeeded","failed","cancelled","compensated"].includes(data.run.status))startPolling(runId);}
...
async function recoverCurrent(){if(!currentRun.value||!canRecover.value)return;try{await ElMessageBox.confirm(`将复用当前运行的 ${checkpoints.value.length} 个 Checkpoint，并按当前已保存工作流继续执行。不会单独绕过 DAG 重跑某个节点。`,`从 Checkpoint 恢复`,{type:"warning",confirmButtonText:"恢复",cancelButtonText:"取消"});const res=await recoverWorkflowRun(currentRun.value.executionId);ElMessage.success(`已从 ${res.checkpointCount} 个 Checkpoint 恢复执行`);startPolling(res.executionId);}catch(e:any){if(e!=="cancel"&&e!=="close")ElMessage.error(e?.response?.data?.error||e?.message||"Checkpoint 恢复失败");}}
async function rerunCurrent(){if(!currentRun.value||!canRerun.value)return;try{const source=currentRun.value.executionId;const res=await rerunWorkflowRun(source,false);currentRun.value={executionId:res.executionId,workflowId:workflow.id,status:"running"};stepRuns.value=[];stepAttempts.value=[];checkpoints.value=[];ElMessage.success("已使用原运行输入重新执行当前已保存工作流");startPolling(res.executionId);}catch(e:any){ElMessage.error(e?.response?.data?.error||e?.message||"重新运行失败");}}
...
onMounted(async()=>{const [loaded,items,ownedWorkflows]=await Promise.all([getWorkflow(workflowId),getWorkflowCatalog().catch(()=>[] as WorkflowCatalogItem[]),listWorkflows().catch(()=>[] as WorkflowDefinition[])]);Object.assign(workflow,loaded);catalog.value=items;nestedWorkflowCandidates.value=ownedWorkflows.filter(item=>item.id!==workflowId);normalizeLoadedDefinition();dirty.value=false;history.value=[modelSnapshot()];historyIndex.value=0;await Promise.all([refreshObservability(),refreshRevisions()]);setTimeout(fitView,0);});
