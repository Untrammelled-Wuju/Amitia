  ...
  <div class="page">
    <div class="page-header">
      ...
      <div v-if="showLayers" class="layer-filters">
        <el-checkbox
          v-model="layerFilters.working"
          label="工作记忆"
          size="small"
          @change="applyFilters"
        />
        <el-checkbox
          v-model="layerFilters.profile"
          label="用户画像"
          size="small"
          @change="applyFilters"
        />
        <el-checkbox
          v-model="layerFilters.episodic"
          label="情景记忆"
          size="small"
          @change="applyFilters"
        />
        <el-checkbox
          v-model="layerFilters.fact"
          label="结构化事实"
          size="small"
          @change="applyFilters"
        />
        <el-checkbox
          v-model="layerFilters.worldbook"
          label="世界书"
          size="small"
          @change="applyFilters"
        />
        <el-checkbox
          v-model="layerFilters.graph"
          label="图谱"
          size="small"
          @change="applyFilters"
        />
      ...
      <div v-if="diffMode" class="diff-bar">
        ...
        <el-button
          size="small"
          @click="doDiff"
          :disabled="!diffTime1 || !diffTime2"
          >对比</el-button
        >
        <span v-if="diffResult" class="diff-summary"
          >新增{{ diffResult.added }} 修改{{ diffResult.modified }} 删除{{
            diffResult.deleted
          }}</span
        >
      </div>
    ...
    <div class="timeline" v-if="items.length > 0">
      ...
      >
        ...
        <div class="tl-card">
          <div class="tl-header">
            <el-tag size="small" :type="tagType(item.event_type)">{{
              eventLabel(item.event_type)
            }}</el-tag>
            <span class="tl-time">{{ formatDate(item.created_at) }}</span>
            <span v-if="item.character_name" class="tl-char">{{
              item.character_name
            }}</span>
          </div>
          <div class="tl-body">
...
function guessLayer(item: any): string {
  if (item.source === "profile" || item.source === "user_profile")
    return "profile";
...
function layerColor(layer: string): string {
  const colors: Record<string, string> = {
    working: "var(--ac-color-primary)",
    profile: "var(--ac-color-warning)",
    episodic: "var(--ac-color-danger)",
    fact: "var(--ac-color-success)",
    worldbook: "var(--ac-color-text-muted)",
    graph: "var(--ac-color-primary-light)",
  };
...
function exportCSV() {
  ...
  a.href = url;
  a.download = "memory-timeline.csv";
  a.click();
...
function doDiff() {
  ...
  const before = allItems.value.filter(
    (i: any) => new Date(i.created_at).getTime() <= t1,
  );
  const after = allItems.value.filter(
    (i: any) => new Date(i.created_at).getTime() <= t2,
  );
...
.tl-dot {
  ...
  height: 12px;
  border-radius: 50%;
  border: 2px solid var(--ac-color-bg);
  z-index: 1;
}
.dot-created {
...
.dot-pending {
  background: var(--ac-color-text-placeholder);
}
