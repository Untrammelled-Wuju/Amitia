...
export function useSpecialEvents() {
  ...
  async function createSpecialEvent(
    ...
    try {
      const url = characterId
        ? "/api/companion/special-events?characterId=" +
  ...
  async function updateSpecialEvent(
    id: number,
    input: any,
    characterId?: string,
  ): Promise<SpecialEvent> {
    ...
    try {
      const url = characterId
        ? `/api/companion/special-events/${id}?characterId=` +
          encodeURIComponent(characterId)
        : `/api/companion/special-events/${id}`;
  ...
  async function deleteSpecialEvent(
    id: number,
    characterId?: string,
  ): Promise<void> {
    loading.value = true;
