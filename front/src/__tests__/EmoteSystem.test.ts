...
describe("表情包前端", () => {
  ...
  it("导入预览、逐项结果、动图预览和详情编辑可用", () => {
    ...
    expect(managerSource).toContain("assetUrl(hoveredId === item.id ? item.filePath : item.thumbnailPath)")
  ...
  it("表情包入口归属角色与记忆且不再出现在设置页签", () => {
    ...
    expect(sideNavSource).toContain('<el-menu-item index="/emotes">表情包管理</el-menu-item>')
    expect(settingsSource).not.toContain("/settings/emotes")
    expect(routerSource).toContain('{ path: "/settings/emotes", redirect: "/emotes" }')
  })
  ...
  it("表情消息渲染原图、状态和降级文本", async () => {
    ;(window as any).amitiaDesktop = { getDeploymentConfig: async () => ({ mode: "local" }) }
    resetRuntimeConnectionCache()
    const wrapper = mount(EmoteMessage, { props: { message: { msgType: "emote", content: "[表情：开心]", altText: "开心", originalAssetReference: "/original.gif", fallbackAssetReference: "/fallback.png", status: "sending", width: 120, height: 80 } } })
    await flushPromises()
    expect(wrapper.get("img").attributes("src")).toBe("http://127.0.0.1:18899/original.gif")
    expect(wrapper.text()).toContain("发送中")
    await wrapper.get("img").trigger("error")
    expect(wrapper.get("img").attributes("src")).toBe("http://127.0.0.1:18899/fallback.png")
    await wrapper.get("img").trigger("error")
  ...
  it("表情缺少原图时直接使用降级图且兼容下划线字段", async () => {
    ;(window as any).amitiaDesktop = { getDeploymentConfig: async () => ({ mode: "local" }) }
    resetRuntimeConnectionCache()
    const wrapper = mount(EmoteMessage, { props: { message: { msg_type: "emote", alt_text: "晚安", fallback_asset_reference: "/fallback.png" } } })
    await flushPromises()
    expect(wrapper.get("img").attributes("src")).toBe("http://127.0.0.1:18899/fallback.png")
    await wrapper.get("img").trigger("error")
  ...
  it("同一回复组严格按消息计划顺序显示", () => {
    ...
    const messages = [
      { id: "text-2", responseGroupId: "response-1", deliverySequence: 3, sequence: 13, createdAt },
      { id: "emote", responseGroupId: "response-1", deliverySequence: 2, sequence: 12, createdAt },
      { id: "text-1", responseGroupId: "response-1", deliverySequence: 1, sequence: 11, createdAt },
    ].sort(compareChatMessages)
    expect(messages.map(message => message.id)).toEqual(["text-1", "emote", "text-2"])
  })
  ...
  it("主动消息保留表情字段并能被完整消息补全", () => {
    ...
    const messages = [{ id: "emote-1", role: "assistant", content: "[表情]", source: "proactive" }]
    expect(mergeChatMessage(messages, proactive)).toBe(true)
    expect(messages[0]).toMatchObject({ msgType: "emote", emoteId: "asset-1", source: "proactive" })
  })

  it("外部实时事件的内容类型和媒体尺寸会统一为前端字段", () => {
    expect(normalizeRealtimeMessage({ content_type: "emote", media_width: 240, media_height: 160 })).toMatchObject({ contentType: "emote", width: 240, height: 160 })
  })
})
