...
Write-Host "=== Extension Unit Tests ===" -ForegroundColor Cyan
...
$goArgs = @("test", "-count=1", "-timeout", "120s", "-short")
if ($Verbose) { $goArgs += "-v" }
...
Write-Host "All extension unit tests passed." -ForegroundColor Green
