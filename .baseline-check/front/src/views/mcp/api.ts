  ...
  if (form.credentialText.trim()) {
    credential =
      form.authType === "bearer_token"
        ? form.credentialText.trim()
        : JSON.parse(form.credentialText);
  }
  return {
    name: form.name.trim(),
...
export async function listMCPServers() {
  return data<MCPServer[]>(await apiClient.get("/api/mcp/servers"));
}
export async function createMCPServer(form: MCPServerForm) {
  return data<MCPServer>(
    await apiClient.post("/api/mcp/servers", payload(form)),
  );
}
export async function updateMCPServer(id: string, form: MCPServerForm) {
  return data<MCPServer>(
    await apiClient.put(
...
export async function deleteMCPServer(id: string) {
  await apiClient.delete(`/api/mcp/servers/${encodeURIComponent(id)}`);
}
export async function connectMCPServer(id: string) {
  await apiClient.post(`/api/mcp/servers/${encodeURIComponent(id)}/connect`);
}
export async function disconnectMCPServer(id: string) {
  await apiClient.post(`/api/mcp/servers/${encodeURIComponent(id)}/disconnect`);
}
export async function reconnectMCPServer(id: string) {
  await apiClient.post(`/api/mcp/servers/${encodeURIComponent(id)}/reconnect`);
}
export async function refreshMCPServer(id: string) {
  await apiClient.post(`/api/mcp/servers/${encodeURIComponent(id)}/refresh`);
}
export async function listMCPTools(id: string) {
  return data<MCPTool[]>(
    await apiClient.get(`/api/mcp/servers/${encodeURIComponent(id)}/tools`),
  );
}
...
export async function listMCPPrompts(id: string) {
  return data<MCPPrompt[]>(
    await apiClient.get(`/api/mcp/servers/${encodeURIComponent(id)}/prompts`),
  );
}
...
export async function listMCPLogs(id: string) {
  return data<MCPAuditLog[]>(
    await apiClient.get(`/api/mcp/servers/${encodeURIComponent(id)}/logs`),
  );
}
export async function listMCPTasks(id: string) {
  return data<MCPTask[]>(
    await apiClient.get(`/api/mcp/servers/${encodeURIComponent(id)}/tasks`),
  );
}
export async function cancelMCPTask(id: string, taskId: string) {
  await apiClient.post(
    `/api/mcp/servers/${encodeURIComponent(id)}/tasks/${encodeURIComponent(taskId)}/cancel`,
  );
}
...
export async function listMCPCapabilities(id: string) {
  return data<MCPServerCapability[]>(
    await apiClient.get(
      `/api/mcp/servers/${encodeURIComponent(id)}/capabilities`,
    ),
  );
}
export async function setMCPCapability(
  id: string,
  ...
  return data<MCPServerCapability>(
    await apiClient.put(
      `/api/mcp/servers/${encodeURIComponent(id)}/capabilities/${capability}`,
...
export async function startMCPOAuth(id: string, resourceUrl: string) {
  ...
  return data<{ authorizationUrl: string; sessionId: string }>(
    await apiClient.post(
      `/api/mcp/servers/${encodeURIComponent(id)}/oauth/start`,
      { resourceUrl, redirectUri, scopes: [] },
    ),
  );
}
export async function revokeMCPOAuth(id: string) {
  await apiClient.post(
    `/api/mcp/servers/${encodeURIComponent(id)}/oauth/revoke`,
  );
}
export async function listMCPInteractions() {
  return data<MCPPendingInteraction[]>(
    await apiClient.get("/api/mcp/interactions"),
  );
}
export async function resolveMCPInteraction(
  ...
  await apiClient.post(
    `/api/mcp/interactions/${encodeURIComponent(id)}/resolve`,
    { action, content },
  );
}
