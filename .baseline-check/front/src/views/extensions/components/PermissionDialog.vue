      ...
      >
        <div class="permission-copy">
          <div class="permission-title">
            ...
            <el-tag :type="riskType(row.risk)" size="small">{{
              riskLabel(row.risk)
            }}</el-tag>
          </div>
          <p>{{ row.description }}</p>
        ...
        <div class="permission-controls">
          ...
          <el-select
            :id="`decision-${row.capability}`"
            v-model="row.decision"
            @change="normalizeScope(row)"
          >
...
const hasHighRisk = computed(() =>
  rows.value.some((row) => row.risk === "high"),
);
