...
<template>
  <main class="workshop-page">
    ...
    <el-alert title="安全边界" type="info" :closable="false" show-icon>
      <template #default
        >所有草案都要经过结构校验、权限确认和沙箱测试，安装后默认保持禁用。</template
      >
    </el-alert>
    ...
    >
      <el-form label-position="top" @submit.prevent="create">
        <el-form-item label="产物类型" required
          ><el-radio-group v-model="productType"
            ><el-radio-button value="workflow">执行型工作流</el-radio-button
            ><el-radio-button value="instructions"
              >指令型 Agent Skill</el-radio-button
            ></el-radio-group
          >
          <p class="helper">
            知识、审查、写作和操作规范请选择指令型；API 与宿主操作请选择工作流。
          </p></el-form-item
        >
          ...
          <el-input
            ...
            :placeholder="
              productType === 'instructions'
                ? '例如：创建一个代码审查流程，用于用户要求检查正确性和安全性时。'
                : '例如：输入城市后，通过 HTTPS 天气接口获取数据并生成简洁摘要。'
            "
            @blur="validateRequirement"
          />
          <p class="helper">
            不要粘贴 API Key。需要凭证时只写 Secret 引用名称。
          </p>
        </el-form-item>
      </el-form>
      <template #footer>
...
async function load() {
  ...
  try {
    ...
    const result = await fetchWorkshopSessions(
      characterId.value,
      page.value,
      pageSize,
    );
    ...
    ElMessage.error(
      error?.response?.data?.detail || error?.message || "工坊会话加载失败",
    );
  } finally {
    loading.value = false;
  }
...
function validateRequirement() {
  requirementError.value = requirement.value.trim()
    ? ""
    : "请描述要创建的 Skill";
...
async function create() {
  ...
  try {
    if (productType.value === "instructions") {
      const preview = await generateWorkshopInstruction(
        requirement.value.trim(),
        characterId.value,
      );
      await ElMessageBox.confirm(
        `将安装指令型 Agent Skill “${preview.definition.name}”。兼容状态：${preview.compatibilityReport.status}。安装后默认禁用，且不会生成或执行 scripts。`,
        "确认工坊产物",
        {
          type:
            preview.compatibilityReport.status === "blocked"
              ? "error"
              : "warning",
          confirmButtonText: "安装为当前角色 Skill",
          cancelButtonText: "取消",
        },
      );
...
async function archive(session: WorkshopSession) {
  await ElMessageBox.confirm(
    "归档只关闭工坊会话，不会删除已安装的 Skill。",
    "确认归档",
    { type: "warning" },
  );
  await archiveWorkshopSession(session.id, characterId.value);
