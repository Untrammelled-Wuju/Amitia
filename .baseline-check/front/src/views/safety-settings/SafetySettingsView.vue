...
<template>
  <div class="safety-page">
    ...
    <el-card shadow="never" class="section-card">
      <template #header
        ><span class="section-title">BDI 硬约束过滤规则</span></template
      >
      <el-alert
        type="info"
        :closable="false"
      ...
      >
        ...
        <el-table-column label="严重程度" width="100">
          <template #default="{ row }">
            <el-tag
              :type="row.severity === 'block' ? 'danger' : 'warning'"
              size="small"
            >
    ...
    <el-card shadow="never" class="section-card">
      <template #header
        ><span class="section-title">BDI 软偏好加权</span></template
      >
      <el-alert
        type="info"
        :closable="false"
        ...
        <el-table-column label="归一化权重" width="110">
          <template #default="{ row }">
            <span style="font-size: 13px">{{
              row.normalizedWeight?.toFixed(2)
            }}</span>
          </template>
        </el-table-column>
      ...
      <div style="margin-top: 10px; display: flex; gap: 8px">
        ...
        <el-button size="small" type="primary" @click="saveBdiConfig"
          >保存偏好设置</el-button
        >
      </div>
    </el-card>
    ...
    <el-card shadow="never" class="section-card">
      <template #header
        ><span class="section-title">应对策略选择</span></template
      >
      <el-alert
        type="info"
        :closable="false"
      ...
      <div class="coping-section">
        ...
        <div class="coping-row">
          ...
          <el-input
            v-model="copingReason"
            type="textarea"
            :rows="2"
            placeholder="描述当前策略的选择理由"
            maxlength="200"
            show-word-limit
            style="flex: 1"
          />
    ...
    <el-card shadow="never" class="section-card">
      <template #header
        ><span class="section-title">情绪表达控制</span></template
      >
      <el-alert
        type="info"
        :closable="false"
      ...
      <div class="emotion-section">
        ...
        <div class="emotion-row">
          ...
          <div class="emotion-slider-group">
            <el-slider
              v-model="emotionInternalIntensity"
              :min="0"
              :max="10"
              :step="0.5"
              style="width: 200px"
        ...
        <div class="emotion-row">
          ...
          <div class="emotion-slider-group">
            <el-slider
              v-model="emotionDisplayIntensity"
              :min="0"
              :max="10"
              :step="0.5"
              style="width: 200px"
        ...
        <div class="emotion-row">
          ...
          <el-input
            v-model="emotionOverrideReason"
            type="textarea"
            :rows="2"
            placeholder="当表达模式非正常显露时，说明原因"
            maxlength="200"
            show-word-limit
            style="flex: 1"
          />
      ...
      <div class="privacy-grid">
        <div class="privacy-card" @click="goPrivacy">
          ...
          <div class="pc-body">
            ...
            <div class="pc-desc">
              查看数据存储位置、模型 API 数据处理方式以及你的控制权说明
            </div>
          </div>
          <el-icon><ArrowRight /></el-icon>
        </div>
        <div class="privacy-card" @click="goBoundary">
          <el-icon size="22" color="var(--ac-color-primary)"
            ><WarningFilled
          /></el-icon>
          <div class="pc-body">
            <div class="pc-title">使用边界</div>
            <div class="pc-desc">
              了解 AI 角色的能力限制、安全边界和理性使用建议
            </div>
          </div>
          <el-icon><ArrowRight /></el-icon>
        </div>
...
async function fetchBdiConfig() {
  try {
    ...
    if (r?.emotionExpression) {
      ...
      emotionInternalIntensity.value =
        r.emotionExpression.internalIntensity ?? 5;
...
async function saveBdiConfig() {
  try {
    await put("/api/safety/bdi-config", {
      ...
      softPreferences: softPreferences.value.map((sp) => ({
        ...sp,
        rawScore: Number(sp.rawScore),
      })),
      copingStrategy: {
...
async function clearEvents() {
  await ElMessageBox.confirm("确定清除所有安全事件日志？", "提示", {
    type: "warning",
  });
  try {
    await del("/api/safety/events");
...
function goPrivacyScan() {
  router.push("/privacy-scan");
}
...
.privacy-card .pc-body {
  flex: 1;
  min-width: 0;
}
.privacy-card .pc-title {
  font-size: 14px;
  font-weight: 600;
  color: var(--ac-color-text);
  margin-bottom: 2px;
}
.privacy-card .pc-desc {
  font-size: 12px;
  color: var(--ac-color-text-muted);
  line-height: 1.4;
}
