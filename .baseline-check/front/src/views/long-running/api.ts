...
export function cleanupTempApi() {
  return post<{ deleted: number; freedBytes: number }>(
    "/api/runtime/cleanup-temp",
  );
}

export function rotateLogsApi() {
  return post<{ rotated: string[]; skipped: string[] }>(
    "/api/runtime/rotate-logs",
  );
}

export function checkDbIntegrityApi() {
  return post<{ ok: boolean; errors: string[] }>(
    "/api/runtime/check-db-integrity",
  );
}
