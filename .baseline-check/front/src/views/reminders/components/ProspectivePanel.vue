  ...
  <el-card shadow="never" class="section-card">
    <template #header>
      <div class="card-header-row">
        ...
        <el-tag
          v-if="queueSummary"
          :type="queueSummary.backpressure ? 'danger' : 'info'"
          size="small"
        >
    ...
    <div v-if="queueSummary" class="queue-stats">
      ...
      <div class="qs-item">
        ...
        <span class="qs-value" style="color: var(--el-color-warning)">{{
      ...
      <div class="qs-item">
        ...
        <span
          ...
          :style="{
            color:
              queueSummary.recentFailures > 0
                ? 'var(--el-color-danger)'
                : 'inherit',
          }"
          >{{ queueSummary.recentFailures }}</span
        >
      </div>
    </div>
    ...
    <div class="prospective-list">
      <div v-for="item in items" :key="item.id" class="prospective-item">
        ...
        <div v-if="item.description" class="pi-desc">
          {{ item.description }}
        </div>
        <div class="pi-meta">
          <span v-if="item.expiresAt" class="pi-meta-item">
            <el-icon style="margin-right: 2px"><Clock /></el-icon>到期:
    ...
    <div style="margin-top: 10px; display: flex; gap: 8px">
      ...
      <el-button
        size="small"
        v-if="queueSummary?.backpressure"
        type="danger"
        plain
        @click="$emit('clearBackpressure')"
      >
        清除背压标记
      </el-button>
    </div>
