...
<template>
  <div class="section-card">
    ...
    <div
      v-if="specialEvents.length === 0"
      class="gender-hint"
      style="padding: 8px 0"
    >
      暂无特殊状态，可添加考试周、兼职、健身、图书馆、生病等
    </div>
    <div v-for="se in specialEvents" :key="se.id" class="course-item">
      <div class="course-info">
        <span class="course-title">{{ se.title }}</span>
        <el-tag size="small" type="info">{{
          SPECIAL_TYPE_LABELS[se.eventType] || se.eventType
        }}</el-tag>
        <span class="course-time" v-if="se.startDate"
          >{{ se.startDate }} ~ {{ se.endDate }}</span
        >
        <span class="course-time" v-if="se.startTime"
          >{{ se.startTime }}-{{ se.endTime }}</span
        >
        <span class="course-reply"
          >回复：{{
            se.replyMode === "NO_REPLY"
              ? "不回复"
              : se.replyMode === "SHORT_REPLY"
                ? "简短"
                : "正常"
          }}</span
        >
      ...
      <div class="course-actions">
        <el-switch
          v-model="se.enabled"
          size="small"
          @change="(v: any) => toggleSpecial(se.id, v)"
        />
        <el-button size="small" @click="editSpecial(se)">编辑</el-button>
        <el-button size="small" type="danger" @click="deleteSpecial(se.id)"
          >删除</el-button
        >
      </div>
    </div>
    ...
    <el-dialog
      v-model="showSpecialDialog"
      :title="editingSpecial ? '编辑特殊状态' : '添加特殊状态'"
      width="500px"
      destroy-on-close
    >
      <div class="course-form">
        ...
        <div class="form-item">
          <label>类型</label>
          <el-select
            v-model="specialForm.eventType"
            size="default"
            style="width: 100%"
          >
            <el-option
              v-for="opt in SPECIAL_EVENT_TYPE_OPTIONS"
              :key="opt.value"
              :label="opt.label"
              :value="opt.value"
            />
        ...
        <div class="form-item">
          <label>回复模式</label>
          <el-select
            v-model="specialForm.replyMode"
            size="default"
            style="width: 100%"
          >
            <el-option
              v-for="opt in SPECIAL_REPLY_OPTIONS"
              :key="opt.value"
              :label="opt.label"
              :value="opt.value"
            />
        ...
        <div class="form-item" v-if="specialForm.eventType === 'SICK_REST'">
          <el-checkbox
            v-model="specialForm.affectSleep"
            label="影响睡眠（提前睡觉）"
          />
        </div>
      </div>
...
import {
  useSpecialEvents,
  SPECIAL_EVENT_TYPE_OPTIONS,
  SPECIAL_REPLY_OPTIONS,
} from "../../../composables/useSpecialEvents";
