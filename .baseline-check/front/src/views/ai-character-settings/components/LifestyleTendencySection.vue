...
<template>
  <div class="section-card">
    <div class="section-title">
      ...
      <el-button
        size="small"
        @click="resetLifestyle"
        :loading="lifestyleResetting"
        type="warning"
        plain
        >恢复默认</el-button
      >
    ...
    <template v-for="group in lifestyleGroups" :key="group.name">
      ...
      <div class="slider-grid">
        <SliderRow
          v-for="item in group.sliders"
          :key="item.key"
          v-model="lifestyleForm[item.key]"
          :label="item.label"
          :left="item.left"
          :right="item.right"
          :min="0"
...
import {
  useLifestyleTendency,
  LIFESTYLE_SLIDERS,
  LIFESTYLE_GROUPS,
} from "../../../composables/useLifestyleTendency";
...
const lifestyleGroups = computed(() =>
