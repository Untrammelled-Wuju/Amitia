    ...
    <div class="ops-grid">
      <div class="op-card">
        <div class="op-info">
          ...
          <div class="op-desc">
            仅重启Wechat Bridge 进程，不影响 Core 服务。适用于 Bridge
            断连或不响应的情况。
          </div>
      ...
      <div class="op-card">
        <div class="op-info">
          ...
          <div class="op-desc">
            仅重启 QQ Bridge 进程，不影响 Core 服务。适用于 QQ Bridge
            断连或不响应的情况。
          </div>
          <div class="op-risk high">高风险操作</div>
        </div>
          ...
          >
            <template #reference>
              <el-button
                type="warning"
                size="small"
                :loading="qqBridgeRestartLoading"
              >
...
async function handleRestartQQBridge() {
  ...
  try {
    ...
    ElMessage.error(
      "QQ Bridge 重启失败: " + (e.response?.data?.message || e.message),
    );
  } finally {
