  ...
  >
    ...
    <div v-else-if="data" class="ctx-preview">
      <div class="ctxp-section">
        ...
        <div class="ctxp-value">
          {{ data.character?.name }} ({{ data.character?.identity }})
        </div>
      </div>
      <div class="ctxp-section">
        <div class="ctxp-label">最近消息</div>
        <div class="ctxp-value">
          {{ data.recentMessageCount }} 条 | 估算总字符:
          {{ data.estimatedChars }}
        </div>
      </div>
      <div class="ctxp-section" v-if="data.usedMemories?.length">
        <div class="ctxp-label">
          使用记忆 ({{ data.usedMemories.length }}条)
        </div>
        <div class="ctxp-memories">
          <div
            v-for="(m, i) in data.usedMemories"
            :key="i"
            class="ctxp-memory-item"
          >
      ...
      <div class="ctxp-section">
        ...
        <div class="ctxp-value ctxp-pre ctxp-prompt">
          {{ data.promptPreview }}
        </div>
      </div>
      <div class="ctxp-section">
        ...
        <div class="ctxp-messages">
          <div
            v-for="(m, i) in data.recentMessages"
            :key="i"
            class="ctxp-msg-item"
            :class="m.role"
          >
            <span class="ctxp-msg-role">{{
              m.role === "user" ? "用户" : "AI"
            }}</span>
