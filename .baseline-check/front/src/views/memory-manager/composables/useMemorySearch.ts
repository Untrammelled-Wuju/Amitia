...
export function useMemorySearch() {
  ...
  async function doSearch() {
    ...
    try {
      ...
      searchResults.value = (result?.items || []).map((item: any) => ({
        id: item.memory?.id || item.id,
        key: item.memory?.key || item.key,
        value: item.memory?.value || item.value,
        memoryType: item.memory?.memoryType || item.memoryType,
        score: item.score ?? 0,
        matchType: item.matchType || "hybrid",
  ...
  async function doGlobalSearch() {
    ...
    try {
      const { useMemoryHub } =
        await import("../../../composables/useMemoryHub");
      const results = await useMemoryHub().globalSearch(
