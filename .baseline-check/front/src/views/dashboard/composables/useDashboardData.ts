...
export function useDashboardData() {
  ...
  const psycheState = ref<import("../../../types").PsycheStateSnapshot | null>(
    null,
  );
  ...
  const deployLabel = computed(() =>
    health.value?.deployMode === "cloud-web" ? "私有云" : "本地桌面",
  );
  const deployClass = computed(() =>
    health.value?.deployMode === "cloud-web" ? "status-warn" : "status-ok",
  );
  const modelLabel = computed(() =>
    health.value?.model === "configured" ? "已配置" : "未配置",
  );
  ...
  const suggestionItems = computed(() => {
    return (
      diagResult.value?.items?.filter(
        (i: any) => i.status !== "ok" && i.suggestion,
      ) || []
    );
  });
  const hasSuggestions = computed(() => {
    return (
      diagResult.value?.items?.some(
        (i: any) => i.status !== "ok" && i.suggestion,
      ) || false
    );
  });
    ...
    try {
      ...
      runtimeHealth.value = {
        ...
        modules: [
          {
            module: "Core",
            status: data?.status === "running" ? "ok" : "warn",
            detail: data?.pid ? `PID: ${data.pid}` : "",
          },
          {
            module: "CPU",
            status: "ok",
            detail: data?.cpu ? `${data.cpu}%` : "",
          },
          {
            module: "Memory",
            status: "ok",
            detail: data?.memory?.rssMB ? `${data.memory.rssMB} MB` : "",
          },
          {
            module: "Uptime",
            status: "ok",
            detail: data?.uptime ? `${Math.floor(data.uptime / 60)}m` : "",
          },
        ],
      };
    } catch {}
        ...
        modules: [
          {
            module: "Core",
            status: "ok",
            detail: data?.startedAt || "Running",
          },
          {
            module: "Check",
            status: data?.started ? "ok" : "warn",
            detail: data?.started ? "Completed" : "Unknown",
          },
        ],
      };
    } catch {
      ...
      if (result) {
        ...
        diagResult.value = {
          ...
          items: checks.map((c: any) => ({
            name: c.name,
            status:
              c.status === "pass" ? "ok" : c.status === "info" ? "ok" : "warn",
            message: c.detail || "",
          })),
  ...
  function getActiveCharacterId(): string {
    ...
    if (stored && characters.value.some((c: any) => c.id === stored))
      return stored;
    const defaultChar =
  ...
  async function fetchPsycheState() {
    ...
    try {
      ...
      if (data && data.emotion) {
        psycheState.value =
          data as import("../../../types").PsycheStateSnapshot;
      }
    } catch {
      psycheState.value = null;
    } finally {
