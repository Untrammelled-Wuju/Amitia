...
<template>
  <el-card shadow="never" class="psyche-card">
    ...
    <div v-if="loading" class="psyche-loading">
      <div class="pl-row" v-for="i in 4" :key="i">
        <div class="pl-bar" :style="{ animationDelay: i * 0.15 + 's' }"></div>
      </div>
    </div>
    <div v-else-if="psyche" class="psyche-grid">
        ...
        <div class="ps-bars">
          <div class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-positive"
                :style="{
                  width: (psyche.emotion.positive * 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
            <span class="ps-bar-val">{{
              (psyche.emotion.positive * 100).toFixed(0)
            }}</span>
          </div>
          <div class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-negative"
                :style="{
                  width: (psyche.emotion.negative * 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
            <span class="ps-bar-val">{{
              (psyche.emotion.negative * 100).toFixed(0)
            }}</span>
          </div>
          <div class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-arousal"
                :style="{
                  width: (psyche.emotion.arousal * 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
            <span class="ps-bar-val">{{
              (psyche.emotion.arousal * 100).toFixed(0)
            }}</span>
          </div>
        </div>
        <div class="ps-affect-label">
          <el-tag size="small" :type="affectTagType">{{
            psyche.affectLabel || "-"
          }}</el-tag>
        </div>
      </div>
      ...
      <div class="ps-section">
        ...
        <div class="ps-bars">
          <div v-if="psyche.beliefs.length === 0" class="ps-empty">
            暂无信念记录
          </div>
          <div
            v-for="b in psyche.beliefs.slice(0, 4)"
            :key="b.key"
            class="ps-belief-row"
          >
            ...
            <div class="ps-belief-conf">
              <div class="ps-bar-track">
                <div
                  class="ps-bar-fill ps-fill-belief"
                  :style="{ width: (b.confidence * 100).toFixed(0) + '%' }"
                ></div>
              </div>
              <span class="ps-bar-val">{{
      ...
      <div class="ps-section">
        ...
        <div class="ps-bars">
          ...
          <div v-for="k in needKeys.slice(0, 5)" :key="k" class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-need"
                :style="{
                  width: Math.min(psyche.needs[k] * 10, 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
      ...
      <div class="ps-section">
        ...
        <div class="ps-bars">
          <div class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-trust"
                :style="{
                  width: (psyche.relationship.trust * 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
            <span class="ps-bar-val">{{
              (psyche.relationship.trust * 100).toFixed(0)
            }}</span>
          </div>
          <div class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-familiar"
                :style="{
                  width:
                    (psyche.relationship.familiarity * 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
            <span class="ps-bar-val">{{
          ...
          <div class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-tension"
                :style="{
                  width: (psyche.relationship.tension * 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
            <span class="ps-bar-val">{{
              (psyche.relationship.tension * 100).toFixed(0)
            }}</span>
          </div>
          <div class="ps-bar-row">
            ...
            <div class="ps-bar-track">
              <div
                class="ps-bar-fill ps-fill-security"
                :style="{
                  width: (psyche.relationship.security * 100).toFixed(0) + '%',
                }"
              ></div>
            </div>
            <span class="ps-bar-val">{{
              (psyche.relationship.security * 100).toFixed(0)
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div v-else class="psyche-empty">
      <el-icon :size="20" color="var(--ac-color-text-muted)"
        ><InfoFilled
      /></el-icon>
...
.pl-bar {
  ...
  background: linear-gradient(
    90deg,
    var(--ac-color-bg-secondary) 0%,
    var(--ac-color-bg-hover) 50%,
    var(--ac-color-bg-secondary) 100%
  );
