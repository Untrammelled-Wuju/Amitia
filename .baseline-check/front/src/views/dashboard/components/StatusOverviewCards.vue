...
const runtimeHealthClass = computed(() =>
  props.runtimeHealth?.overall === "ok"
    ? "status-ok"
    : props.runtimeHealth?.overall === "warning"
      ? "status-warn"
      : "status-off",
);
...
const runtimeHealthLabel = computed(() =>
  props.runtimeHealth?.overall === "ok"
    ...
    : props.runtimeHealth?.overall === "warning"
      ? "注意"
      : props.runtimeHealth?.overall === "error"
        ? "异常"
        : "未知",
);
</script>
...
.status-warn .sc-value {
  background: var(--console-value-off-bg);
