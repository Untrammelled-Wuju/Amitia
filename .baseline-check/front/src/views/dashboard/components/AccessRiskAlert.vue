...
<template>
  <div>
    <div
      v-if="
        accessRisk &&
        (accessRisk.overallLevel === 'error' ||
          accessRisk.overallLevel === 'warn')
      "
      class="access-risk-alert"
      :class="'risk-' + accessRisk.overallLevel"
    >
      ...
      <div class="ara-list">
        <div
          v-for="c in accessRisk.checks.filter((x: any) => x.level !== 'ok')"
          :key="c.name"
          class="ara-item"
          :class="'ara-' + c.level"
        >
      ...
      <div class="ara-footer">
        <router-link to="/settings#access-protection" class="ara-link"
          >前往访问保护设置 →</router-link
        >
      </div>
    </div>
...
.risk-error .ara-header {
  color: var(--ac-color-danger);
}
.risk-warn .ara-header {
