<template>
  <div class="ob-stage-inner">
    <div
      ...
      ref="memorySceneRef"
      :data-identity-state="complete ? 'complete' : 'filling'"
    >
        ...
        <div v-if="!complete" key="incomplete">
          ...
          <div class="ob-identity-ledger" ref="memoryLedgerRef">
            ...
            <div class="ob-identity-ledger-items">
              ...
              <div
                class="ob-identity-ledger-card"
                :class="{ filled: memoryAvatarUploaded }"
              >
                <span class="ob-identity-ledger-key">头像</span>
                <span class="ob-identity-ledger-value">{{
                  memoryAvatarUploaded ? "已上传" : "等待上传"
                }}</span>
              </div>
            </div>
          </div>
          ...
          <div
            v-show="isMemoryAvatarStep"
            class="ob-identity-avatar-area ob-memory-avatar-area"
            ref="memoryAvatarRef"
          >
            <div
              class="ob-identity-avatar-circle"
              :class="{ 'has-image': memoryAvatarPreviewUrl }"
              @click="triggerFileInput"
            >
              <img
                v-if="memoryAvatarPreviewUrl"
                :src="memoryAvatarPreviewUrl"
                alt="用户头像预览"
              />
              <span v-else class="ob-identity-avatar-placeholder">+</span>
            </div>
            <p class="ob-identity-avatar-hint">
              {{ memoryAvatarPreviewUrl ? "点击更换头像" : "点击上传头像" }}
            </p>
            <input
              ref="fileInputRef"
              type="file"
            ...
            <div class="ob-identity-avatar-actions">
              <button class="ob-skip-btn" @click="$emit('memoryAvatarSkip')">
                跳过
              </button>
              <button
                class="ob-primary-ghost ob-avatar-continue-btn"
                :disabled="!memoryAvatarPreviewUrl"
                @click="$emit('memoryAvatarContinue')"
              >
                继续
              </button>
            </div>
            ...
            <div class="ob-identity-complete-card">
              <div class="ob-identity-complete-copy-block">
                <p class="ob-identity-complete-message">
                  ...
                  <span
                    >以后，用
                    <strong>{{ items[0] || "未记录" }}</strong> 称呼你。</span
                  >
                  <span
                    >交流时保持
                    <strong>{{ items[1] || "未记录" }}</strong> 的风格。</span
                  >
                  <span v-if="items[2]"
                    >你会希望我记得：<strong>{{ items[2] }}</strong
                    >。</span
                  >
                </p>
                <div class="ob-identity-complete-footer">
                  <span class="ob-identity-complete-kicker"
                    >初始信息已保存</span
                  >
...
function onTransitionEnter(_el: Element) {
  nextTick(() => {
    const elements = [
      memoryLedgerRef.value,
      memoryAnswerRef.value,
      memoryQuickRef.value,
      memoryPromptRef.value,
      memoryAvatarRef.value,
    ];
