...
<template>
  <div class="graph-page">
    ...
    <div class="graph-controls">
      ...
      <el-button size="small" type="primary" @click="fetchGraph"
        >查询</el-button
      >
    ...
    <div class="graph-stats" v-if="stats">
      ...
      <span
        ...
        :key="t.entity_type"
        class="stat-type"
        :style="{ color: typeColor(t.entity_type) }"
        >{{ typeLabel(t.entity_type) }}: {{ t.count }}</span
      >
    </div>
...
const typeLabels: Record<string, string> = {
  ...
  episodic: "情景",
  worldbook: "世界书",
  worldbook_trigger: "触发片段",
  user: "用户",
  character: "角色",
  entity: "实体",
};
const typeColors: Record<string, string> = {
  memory: "var(--ac-color-primary)",
  profile: "var(--ac-color-warning)",
  episodic: "var(--ac-color-danger)",
  worldbook: "var(--ac-color-success)",
  worldbook_trigger: "var(--ac-color-text-muted)",
  user: "var(--ac-color-primary-light)",
  character: "var(--ac-color-primary-dark)",
  entity: "var(--ac-color-text-secondary)",
};
function typeLabel(t: string) {
  return typeLabels[t] || t;
}
function typeColor(t: string) {
  return typeColors[t] || "var(--ac-color-text-muted)";
}
const typeOptions = computed(() =>
  (stats.value?.byType || []).map((t: any) => ({
    label: typeLabel(t.entity_type),
    value: t.entity_type,
  })),
);
...
const filteredNodes = computed(() => {
  return allNodes.value.filter((n: any) => {
    ...
    if (labelKeyword.value && !(n.label || "").includes(labelKeyword.value))
      return false;
    return true;
  });
});
const filteredNodeIds = computed(
  () => new Set(filteredNodes.value.map((n: any) => extractId(n.id))),
);
const filteredLinks = computed(() =>
  allLinks.value.filter((l: any) => {
    ...
    return (
      filteredNodeIds.value.has(source) && filteredNodeIds.value.has(target)
    );
  }),
);
      ...
      try {
        ...
        if (Array.isArray(allEdgesData) && allEdgesData.length > 0) {
          var nodeIdSet = new Set(
            allNodes.value.map(function (n) {
              return extractId(n.id);
            }),
          );
          allLinks.value = allEdgesData.filter(function (e) {
            return (
              nodeIdSet.has(extractId(e.in)) && nodeIdSet.has(extractId(e.out))
            );
          });
          renderGraph();
        }
...
async function renderGraph() {
  ...
  var links = filteredLinks.value.map((l: any) => {
    return {
      source: extractId(l.in || l.source),
      target: extractId(l.out || l.target),
    };
  });
...
.graph-container.fullscreen .chart {
