    ...
    <div v-if="memories.length > 0">
      <div v-for="m in memories" :key="m.key" class="memory-card">
        <div class="card-tags">
          ...
          <el-tag v-if="m.confidence >= 80" size="small" type="success"
            >高置信</el-tag
          >
          <el-tag v-else-if="m.confidence >= 50" size="small" type="warning"
            >中置信</el-tag
          >
          <el-tag v-else size="small" type="danger">低置信</el-tag>
          <el-tag
            v-if="m.verifiedStatus === 'user_verified'"
            size="small"
            type="success"
            effect="dark"
            >✓</el-tag
          >
          <el-tag
            v-else-if="m.verifiedStatus === 'contradicted'"
            size="small"
            type="danger"
            effect="dark"
            >⚠</el-tag
        ...
        <div v-if="m.expiresAt" class="memory-expiry">
          过期: {{ m.expiresAt }}
        </div>
      </div>
    </div>
