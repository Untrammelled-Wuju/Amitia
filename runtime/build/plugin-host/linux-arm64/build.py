...
TS_CONFIG = os.path.join(PLUGIN_HOST_ROOT, "tsconfig.json")
PACKAGE_JSON = os.path.join(PLUGIN_HOST_ROOT, "package.json")

OUTPUT_DIR_NAME = "plugin-host"
FROZEN_RECORD_NAME = "plugin-host-frozen-record.json"
...
def find_tsc(node_bin, plugin_host_dir):
    if node_bin:
        ...
        if os.path.isfile(local_tsc):
            return local_tsc
    local_npm_modules = os.path.join(plugin_host_dir, NODE_MODULES_DIR, "typescript", "bin", "tsc")
    if os.path.isfile(local_npm_modules):
        return local_npm_modules
    return None


def build_plugin_host(input_dir, output_root, node_bin=None):
    if not os.path.isdir(SRC_DIR):
        raise BuildError(f"Plugin host source directory not found: {SRC_DIR}")
    ...
    try:
        ...
        if validation_errors:
            raise BuildError(f"Plugin host record validation failed: {'; '.join(validation_errors)}")

        frozen_path = os.path.join(staging, FROZEN_RECORD_NAME)
        record.write(frozen_path)

        manifest_path = os.path.join(staging, "plugin-host-files.sha256")
...
def main():
    ...
    if not args.output:
        print("Plugin host builder: ERROR - --output directory required", file=sys.stderr)
        return 1

    try:
        record = build_plugin_host(args.input, args.output, node_bin=args.node_bin)
        print(f"Plugin host builder: completed. Version: {record.version}")
        return 0
    except BuildError as e:
        print(f"Plugin host builder: BUILD ERROR - {e}", file=sys.stderr)
        return 1
    except Exception as e:
        print(f"Plugin host builder: UNEXPECTED ERROR - {e}", file=sys.stderr)
        return 2
