...
def build(
    ...
    for script in REQUIRED_SCRIPTS:
        script_path = source_path / script
    ...
    publish_base = os.path.join(output_base, "runtime-scripts", "linux-arm64")
    ...
    record = (
        BuildRecordBuilder("runtime-scripts", version)
        .set_target("linux", "arm64")
        .set_artifact_type("frozen-tree")
        .set_artifact_path(
            f"runtime-scripts/linux-arm64/{version}",
            published_dir,
        )
...
def main():
    parser = argparse.ArgumentParser(description="Runtime Scripts linux-arm64 Frozen Builder")
