...
BuildSpec = importlib.util.spec_from_file_location("runtime_scripts_linux_arm64_build", str(SCRIPT_DIR / "build.py"))
BuildModule = importlib.util.module_from_spec(BuildSpec)
sys.modules["runtime_scripts_linux_arm64_build"] = BuildModule
...
class TestFreshBuild(unittest.TestCase):
    def setUp(self):
        ...
        self.pub_dir = setup_pub_dir(self.output_base, "runtime-scripts", "1.0.0")
        self.patches = [
            mock.patch("runtime_scripts_linux_arm64_build.sha256_tree_manifest", return_value="a" * 64),
    ...
    def test_fresh_build_returns_record(self):
        with mock.patch("runtime_scripts_linux_arm64_build.atomic_publish_directory") as mock_pub:
...
class TestShaMismatch(unittest.TestCase):
    ...
    def test_post_publish_tree_sha_mismatch_fails(self):
        ...
        with mock.patch("runtime_scripts_linux_arm64_build.sha256_tree_manifest", side_effect=fake_tree):
...
class TestSameVersionSameBytes(unittest.TestCase):
    ...
    def test_same_version_same_tree_reuses_or_succeeds(self):
        pub_dir = setup_pub_dir(self.output_base, "runtime-scripts", "1.0.0")
        with mock.patch("runtime_scripts_linux_arm64_build.atomic_publish_directory") as mock_pub:
            mock_pub.return_value = mock.Mock(
                success=True, published_dir=str(pub_dir), errors=[]
            )
...
class TestSameVersionDifferentBytes(unittest.TestCase):
    ...
    def test_existing_publish_dir_causes_failure(self):
        ...
        with mock.patch("runtime_scripts_linux_arm64_build.sha256_tree_manifest", return_value="x" * 64):
            with mock.patch("common.artifact_record.sha256_file", return_value="b" * 64):
                with mock.patch("runtime_scripts_linux_arm64_build.atomic_publish_directory") as mock_pub:
                    mock_pub.return_value = mock.Mock(
                        success=False, published_dir="",
...
class TestFailureDuringCandidateMetadata(unittest.TestCase):
    ...
    def test_publish_failure_leaves_no_partial_record(self):
        with mock.patch("runtime_scripts_linux_arm64_build.sha256_tree_manifest", return_value="a" * 64):
            with mock.patch("common.artifact_record.sha256_file", return_value="b" * 64):
                with mock.patch("runtime_scripts_linux_arm64_build.atomic_publish_directory") as mock_pub:
                    mock_pub.return_value = mock.Mock(
...
class TestReproducibility(unittest.TestCase):
    ...
    def test_two_builds_same_tree_sha(self):
        pub_dir1 = setup_pub_dir(self.output_base, "runtime-scripts", "1.0.0")
        with mock.patch("runtime_scripts_linux_arm64_build.atomic_publish_directory") as mock_pub:
            mock_pub.return_value = mock.Mock(
        ...
        output_base2.mkdir()
        pub_dir2 = setup_pub_dir(output_base2, "runtime-scripts", "1.0.0")
        with mock.patch("runtime_scripts_linux_arm64_build.atomic_publish_directory") as mock_pub:
            mock_pub.return_value = mock.Mock(
