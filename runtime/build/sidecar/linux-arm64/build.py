...
def build(
    ...
    bundle = source_path / "bundle.mjs"
    launcher = source_path / "launcher.mjs"
    if not bundle.exists():
        raise FileNotFoundError(f"sidecar bundle.mjs not found: {bundle}")
    if not launcher.exists():
        raise FileNotFoundError(f"sidecar launcher.mjs not found: {launcher}")
    ...
    publish_base = os.path.join(output_base, "sidecar", "linux-arm64")
    ...
    record = (
        BuildRecordBuilder("sidecar", version)
        .set_target("linux", "arm64")
        .set_artifact_type("frozen-tree")
        .set_artifact_path(
            f"sidecar/linux-arm64/{version}",
            published_dir,
        )
...
def main():
    parser = argparse.ArgumentParser(description="Sidecar linux-arm64 Frozen Builder")
