#!/usr/bin/env python3
"""Canonical Sidecar Builder for Linux ARM64."""
import argparse
import json
import os
import shutil
...
TS_CONFIG = os.path.join(SIDECAR_ROOT, "tsconfig.json")
PACKAGE_JSON = os.path.join(SIDECAR_ROOT, "package.json")

OUTPUT_DIR_NAME = "sidecar"
FROZEN_RECORD_NAME = "sidecar-frozen-record.json"
...
def find_tsc(node_bin, sidecar_dir):
    if node_bin:
        ...
        if os.path.isfile(local_tsc):
            return local_tsc
    local_npm_modules = os.path.join(sidecar_dir, NODE_MODULES_DIR, "typescript", "bin", "tsc")
    if os.path.isfile(local_npm_modules):
        return local_npm_modules
    return None


def build_sidecar(input_dir, output_root, node_bin=None):
    if not os.path.isdir(SRC_DIR):
        raise BuildError(f"Sidecar source directory not found: {SRC_DIR}")
    ...
    try:
        ...
        if validation_errors:
            raise BuildError(f"Sidecar record validation failed: {'; '.join(validation_errors)}")

        frozen_path = os.path.join(staging, FROZEN_RECORD_NAME)
        record.write(frozen_path)

        manifest_path = os.path.join(staging, "sidecar-files.sha256")
...
def main():
    parser = argparse.ArgumentParser(description="Sidecar Linux ARM64 Builder")
    ...
    if not args.output:
        print("Sidecar builder: ERROR - --output directory required", file=sys.stderr)
        return 1

    try:
        record = build_sidecar(args.input, args.output, node_bin=args.node_bin)
        print(f"Sidecar builder: completed. Version: {record.version}")
        return 0
    except BuildError as e:
        print(f"Sidecar builder: BUILD ERROR - {e}", file=sys.stderr)
        return 1
    except Exception as e:
        print(f"Sidecar builder: UNEXPECTED ERROR - {e}", file=sys.stderr)
        return 2
