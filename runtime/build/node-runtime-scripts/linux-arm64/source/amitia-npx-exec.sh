...
require_regular_file "${AMITIA_NPX_CLI}" "npx_cli"
...
_AMITIA_NPX_ARGS=""
_AMITIA_HAS_NO=false
for _arg in "$@"; do
  if [ "${_arg}" = "--yes=false" ] || [ "${_arg}" = "--no-install" ] || [ "${_arg}" = "-n" ]; then
    _AMITIA_HAS_NO=true
  fi
  _AMITIA_NPX_ARGS="${_AMITIA_NPX_ARGS} ${_arg}"
done

if [ "${_AMITIA_HAS_NO}" = "false" ]; then
  _AMITIA_NPX_ARGS="--no-install${_AMITIA_NPX_ARGS}"
fi

set -- ${_AMITIA_NPX_ARGS}
...
exec env -i \
  ...
  "${AMITIA_NODE_BIN}" "${AMITIA_NPX_CLI}" "$@"
