...
def build(
    ...
    if not entrypoint.exists():
        raise FileNotFoundError(f"Task Host entrypoint not found: {entrypoint}")
    ...
    publish_base = os.path.join(output_base, "task-host", "linux-arm64")
    ...
    record = (
        BuildRecordBuilder("task-host", version)
        .set_target("linux", "arm64")
        .set_artifact_type("frozen-tree")
        .set_artifact_path(
            f"task-host/linux-arm64/{version}",
            published_dir,
        )
...
def main():
    parser = argparse.ArgumentParser(description="Task Host linux-arm64 Frozen Builder")
