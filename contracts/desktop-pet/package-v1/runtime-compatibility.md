...
Schema 1 不再兼容 `minimumRuntimeVersion` 写法。
...
Runtime v1 的握手契约版本独立记录在 `desktopPetRuntimeContractVersion`，构建门禁必须校验其与后端 `CurrentSchemaVersion` 一致；服务端在 `Hello` 阶段拒绝不匹配版本，禁止先进入 Ready 再延迟失败。
