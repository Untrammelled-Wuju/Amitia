...
class RealtimeAudioBridge {
  static const MethodChannel _control = MethodChannel(
    'com.amitia.realtime_audio/control',
  );
  static const EventChannel _input = EventChannel(
    'com.amitia.realtime_audio/input',
  );
  ...
  Stream<Uint8List> get inputPcm {
    return _cachedInput ??= _input.receiveBroadcastStream().map((event) {
      ...
      throw StateError(
        'Unsupported realtime audio payload: ${event.runtimeType}',
      );
    }).asBroadcastStream();
  }
  ...
  Future<void> stopPlayback() async {
    await _control.invokeMethod<void>('stopPlayback');
  }
  ...
  Future<void> emitWorkflowAsrFinal({
    ...
    try {
      await _control
          .invokeMethod<void>('emitWorkflowASRFinal', <String, dynamic>{
