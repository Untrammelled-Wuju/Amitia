...
class _SafetyContentState extends ConsumerState<_SafetyContent> {
  ...
  Future<void> _showBdiConfig() async {
    try {
      final current =
          await ref.read(safetyServiceProvider).bdiConfig() ??
          <String, dynamic>{};
      if (!mounted) return;
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '可配置 hardConstraints、softPreferences、copingStrategy 和 emotionExpression。保存时由后端统一持久化。',
              ...
              onPressed: () async {
                try {
                  ...
                  await ref
                      ...
                      .updateBdiConfig(Map<String, dynamic>.from(decoded));
      ...
      if (saved == true && mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('BDI 安全策略已保存')));
      }
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('读取 BDI 配置失败：$e')));
    }
  }
  ...
  Future<void> _showSafetyEvents() async {
    try {
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Text(
                          '安全事件 (${result['total'] ?? items.length})',
                          style: AppTypography.sectionTitle(sheetContext),
                        ),
                      ),
                      TextButton.icon(
                        onPressed: items.isEmpty
                            ...
                            : () async {
                                ...
                                if (mounted)
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('安全事件已清空')),
                                  );
                              },
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (_, index) {
                            ...
                            final createdAt = (item['createdAt'] ?? '')
                                .toString();
                            final handled =
                                item['handled'] == true || item['handled'] == 1;
                            return ListTile(
                              leading: Icon(
                                handled
                                ...
                                color: handled
                                    ? sheetContext.success
                                    : sheetContext.warning,
                              ),
                              title: Text(
                                eventType,
                                style: AppTypography.body(sheetContext),
      ...
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('读取安全事件失败：$e')));
    }
  }
    ...
    return ListView(
      ...
      children: [
        ...
        _buildCard([
          _switch(
            '防止情感勒索',
            '拦截以 guilt、威胁或压力强迫用户互动的表达',
            _preventEmotionalBlackmail,
          ...
          _switch(
            '限制敏感主动提及',
            '主动消息中减少敏感主题触发',
            _preventSensitiveProactiveMention,
    ...
    trailing: SizedBox(
      ...
      child: TextFormField(
        ...
        onChanged: (value) {
          ...
          if (parsed != null)
            _auditLogRetentionDays = parsed.clamp(1, 3650).toInt();
        },
      ),
    ),
  );
