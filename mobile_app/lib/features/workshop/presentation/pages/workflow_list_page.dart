...
WorkflowApiTarget _workflowTargetFromKey(String key) {
  ...
  if (key.startsWith('device:'))
    return WorkflowApiTarget.device(key.substring('device:'.length));
  return const WorkflowApiTarget.cloud();
}

final _workflowListProvider =
    FutureProvider.family<List<Map<String, dynamic>>, String>((ref, key) async {
      ...
      return ref
          ...
          .workflows(limit: 200, target: _workflowTargetFromKey(key));
    });
...
class _WorkflowListPageState extends ConsumerState<WorkflowListPage> {
  ...
  String get _targetKey =>
      _location == 'device' ? 'device:$_deviceId' : _location;
  ...
  String _editorRoute(String id) =>
      AppRoutes.workflowEditor(id, location: _location, deviceId: _deviceId);
    ...
    try {
      ...
      setState(() {
        ...
        final exists = items.any(
          (item) => (item['deviceId'] ?? '').toString() == _deviceId,
        );
        if (!exists)
          _deviceId = items.isEmpty
              ? ''
              : (items.first['deviceId'] ?? '').toString();
      });
    } catch (_) {
  ...
  Future<void> _primeSyncCursor() async {
    ...
    try {
      final page = await ref
          .read(extensionServiceProvider)
          .workflowSyncEvents(target: _target);
  ...
  Future<void> _pollSyncEvents() async {
    ...
    try {
      ...
      final page = await ref
          .read(extensionServiceProvider)
          .workflowSyncEvents(target: _target, afterCursor: _syncCursor);
      if (!mounted || _targetKey != key) return;
      final rawCursor = page['cursor'];
      _syncCursor = rawCursor is int
          ? rawCursor
          : int.tryParse(rawCursor?.toString() ?? '') ?? _syncCursor;
      final items = page['items'];
      final changed = items is List && items.isNotEmpty;
      if (changed) _refresh();
      _deviceRefreshTicks++;
      if (_location != 'local' && _deviceRefreshTicks % 5 == 0)
      ...
      if (_location == 'device' && !changed && _deviceRefreshTicks % 8 == 0)
        _refresh();
    } catch (_) {
  ...
  Future<void> _createWithAI() async {
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            hintText: '描述想自动化的流程，例如：每天早上获取天气，如果下雨就通知我带伞。',
          ),
        ),
        actions: [
          TextButton(
    ...
    try {
      final service = ref.read(extensionServiceProvider);
      final proposal = await service.generateWorkflowWithAI(
        instruction.trim(),
        target: _target,
      );
  ...
  Future<void> _duplicate(Map<String, dynamic> item) async {
    ...
    final created = await ref
        .read(extensionServiceProvider)
        .duplicateWorkflow(id, target: _target);
  ...
  Future<void> _delete(Map<String, dynamic> item) async {
    ...
    await ref
        .read(extensionServiceProvider)
        .deleteWorkflow(id, target: _target);
    _selected.remove(id);
    _refresh();
    if (mounted) setState(() {});
    _show('工作流已删除');
  ...
  Future<void> _showTemplates() async {
    final templates = await ref
        .read(extensionServiceProvider)
        .workflowTemplates(target: _target);
    if (!mounted) return;
  ...
  String _safeName(String value) {
    ...
    return sanitized.isEmpty
        ? 'workflow'
        : sanitized.substring(0, sanitized.length > 80 ? 80 : sanitized.length);
  }
  ...
  Future<void> _export(Map<String, dynamic> item) async {
    ...
    final envelope = await ref
        .read(extensionServiceProvider)
        .exportWorkflow(id, target: _target);
    final output = await FilePicker.platform.saveFile(
      dialogTitle: '导出工作流',
      fileName:
          '${_safeName((item['name'] ?? 'workflow').toString())}.workflow.json',
  ...
  Future<void> _import() async {
    ...
    try {
      ...
      final created = await ref
          .read(extensionServiceProvider)
          .importWorkflow(decoded.cast<String, dynamic>(), target: _target);
      final id = (created['id'] ?? '').toString();
      _refresh();
  ...
  Future<void> _batchEnable(bool enabled) async {
    ...
    try {
      for (final id in _selected) {
        await ref
            .read(extensionServiceProvider)
            .setWorkflowEnabled(id, enabled, target: _target);
      }
      _selected.clear();
      _refresh();
  ...
  List<Widget> _workflowCards(List<Map<String, dynamic>> items) {
    return items
        .map((item) {
          final id = (item['id'] ?? '').toString();
          return Padding(
            ...
            child: _WorkflowCard(
              ...
              onOpen: item['offline'] == true
                  ? () => _show('设备离线：当前只能查看最后已知目录，完整 Definition 需要设备上线后读取')
                  : () => context.push(_editorRoute(id)),
              onRun: item['enabled'] == true && item['offline'] != true
                  ? () => _run(id)
                  : null,
              onToggle: (value) async {
                if (item['offline'] == true) {
                  _show('设备离线：不能修改本地工作流');
                  return;
                }
                await ref
                    .read(extensionServiceProvider)
                    .setWorkflowEnabled(
                      ...
                      value,
                      target: _target,
                      expectedRevision: _revisionOf(item),
                    );
                _refresh();
              },
              onDuplicate: _location == 'device'
                  ? () => _show('远程设备控制面不提供复制')
                  : () => _duplicate(item),
              onTemplate: _location == 'device'
                  ? () => _show('远程设备控制面不提供模板管理')
                  : () => _saveTemplate(item),
              onExport: _location == 'device'
                  ? () => _show('远程设备离线目录不提供导出')
                  : () => _export(item),
              onDelete: item['offline'] == true
                  ? () => _show('设备离线：不能删除本地工作流')
                  : () => _delete(item),
            ),
          );
        })
  ...
  Widget _buildSearchView(List<Map<String, dynamic>> items) {
        ...
        Expanded(
          child: query.isEmpty
              ...
              : ListView(
                  ...
                  children: _workflowCards(results),
                ),
        ),
      ],
    );
  ...
  Widget build(BuildContext context) {
    ...
    return AmitiaScaffold(
      appBar: AmitiaAppBar(
        title: _searchVisible ? '搜索工作流' : '工作流',
        showBackButton: true,
        ...
        actions: _searchVisible
            ...
            : [
                ...
                IconButton(
                  tooltip: '导入 JSON',
                  onPressed: _busy || _location == 'device' ? null : _import,
                  icon: const Icon(Icons.file_download_outlined),
                ),
                IconButton(
                  tooltip: '我的模板',
                  onPressed: _busy || _location == 'device'
                      ? null
                      : _showTemplates,
      ...
      body: SafeArea(
        ...
        child: asyncItems.when(
          ...
          data: (items) {
            if (_searchVisible) return _buildSearchView(items);
                ...
                children: [
                  SegmentedButton<String>(
                    ...
                    onSelectionChanged: (values) async {
                      ...
                      setState(() {
                        _location = values.first;
                        _selected.clear();
                      });
                      if (_location == 'device' && _devices.isEmpty)
                        await _loadDevices();
                    ...
                    Container(
                      ...
                      child: Wrap(
                        ...
                        children: [
                          ...
                          OutlinedButton(
                            onPressed: _busy ? null : () => _batchEnable(true),
                            child: const Text('启用'),
                          ),
                          OutlinedButton(
                            onPressed: _busy ? null : () => _batchEnable(false),
                            child: const Text('停用'),
                          ),
                          OutlinedButton(
                  ...
                  if (filtered.isEmpty)
                    _EmptyState(
                      ...
                      onCreate: _create,
                      onCreateAI: _location == 'device'
                          ? () => _show('远程设备控制面不提供 AI 创建')
                          : _createWithAI,
                      onTemplates: _location == 'device'
                          ? () => _show('远程设备控制面不提供模板管理')
                          : _showTemplates,
                    )
                  else
                    ..._workflowCards(filtered),
                ],
              ),
            );
          },
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: AppRadius.brLarge,
            border: Border.all(
              color: selected ? context.accentPrimary : context.borderPrimary,
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  if (item['callableByAgent'] == true)
                    const _Pill(label: 'Agent Tool'),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            hasAny ? '没有匹配的工作流' : '还没有工作流',
            style: AppTypography.cardTitle(context),
          ),
          const SizedBox(height: 6),
          Text(
            hasAny ? '调整搜索或筛选条件。' : '从空白、AI 或我的模板开始。',
            style: AppTypography.caption(context),
          ),
          if (!hasAny) ...[
