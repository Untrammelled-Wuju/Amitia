  ...
  <el-card shadow="never" class="section-card">
    ...
    <el-table :data="rules" stripe size="small" v-loading="loading">
      ...
      <el-table-column label="渠道" width="90">
        <template #default="{ row }">
          <el-tag
            ...
            size="small"
          >
            <span :title="channelLabel(row.channel)">{{
      ...
      <el-table-column label="安静时段" width="110">
        <template #default="{ row }"
          >{{ row.quietStart }} - {{ row.quietEnd }}</template
        >
      </el-table-column>
      <el-table-column label="今日/上限" width="80">
        <template #default="{ row }"
          >{{ row.sentCountToday }}/{{ row.maxPerDay }}</template
        >
      </el-table-column>
      ...
      <el-table-column label="操作" width="260" fixed="right">
        <template #default="{ row }">
          <el-button link size="small" @click="emit('edit', row)"
            >编辑</el-button
          >
          <el-button link size="small" type="warning" @click="emit('test', row)"
            >测试</el-button
          >
          <el-button
            link
