...
export type LifestyleTendencyInput = Partial<
  Pick<
    LifestyleTendencyDTO,
    | "punctualityTendency"
    | "earlyPrepareTendency"
    | "selfDisciplineTendency"
    | "sleepinessTendency"
    | "randomnessTendency"
    | "activityEnergy"
    | "socialEnergy"
...
export const LIFESTYLE_GROUPS = [
  ...
  {
    name: "主动互动",
    keys: ["socialEnergy", "careTendency", "dailyShareTendency"],
  },
];
...
export function useLifestyleTendency() {
  ...
  async function getLifestyleTendency(
    characterId?: string,
  ): Promise<LifestyleTendencyDTO> {
    loading.value = true;
    try {
      const params: any = {};
      if (characterId) params.characterId = characterId;
      const data = await get<LifestyleTendencyDTO>(
  ...
  async function updateLifestyleTendency(
    input: LifestyleTendencyInput,
    characterId?: string,
  ): Promise<LifestyleTendencyDTO> {
    loading.value = true;
    try {
      const url = characterId
        ? "/api/companion/lifestyle-tendency?characterId=" +
  ...
  async function resetLifestyleTendency(
    characterId?: string,
  ): Promise<LifestyleTendencyDTO> {
    loading.value = true;
    try {
      const url = characterId
        ? "/api/companion/lifestyle-tendency/reset?characterId=" +
          encodeURIComponent(characterId)
