...
type V1RuntimeActionAdapter struct {
	facade *runtimev1.RuntimeFacade
}

func NewV1RuntimeActionAdapter(facade *runtimev1.RuntimeFacade) *V1RuntimeActionAdapter {
	return &V1RuntimeActionAdapter{facade: facade}
}

func (a *V1RuntimeActionAdapter) SubmitBehaviorCommand(ctx context.Context, cmd behavior.BehaviorRuntimeCommand) (*behavior.CommandReceipt, error) {
	if a.facade == nil {
		return &behavior.CommandReceipt{
			CommandID:  "",
			Accepted:   false,
			Status:     behavior.CmdOffline,
			Error:      "v1 runtime facade unavailable",
			ReceivedAt: time.Now(),
		}, behavior.NewBehaviorError(behavior.ErrCodeRuntimeOffline, "v1 runtime facade unavailable")
	}

	queuePolicy := runtimev1.PlayActionQueueEnqueue
	...
	if err != nil && !duplicate {
		log.Logger.Warnf("wiring/runtime_v1_action_port: create command failed decisionId=%s actionKey=%s err=%v",
	...
	if duplicate {
		status := runtimev1.CommandStatus(v2Cmd.Status)
		switch status {
		case runtimev1.CommandStatusCompleted:
			// The exact idempotent command already completed. Treat this as a
			// successful replay and never mutate a terminal Runtime V1 row.
			return acceptedRuntimeReceipt(v2Cmd.ID), nil
		case runtimev1.CommandStatusFailedTerminal, runtimev1.CommandStatusExpired,
			runtimev1.CommandStatusCancelRequested, runtimev1.CommandStatusCancelled,
			runtimev1.CommandStatusSuperseded:
		...
		case runtimev1.CommandStatusDispatching, runtimev1.CommandStatusTransportDispatched,
			runtimev1.CommandStatusRuntimeReceived, runtimev1.CommandStatusRuntimeAccepted,
			runtimev1.CommandStatusRendererAccepted, runtimev1.CommandStatusPlaybackStarted:
			...
			return acceptedRuntimeReceipt(v2Cmd.ID), nil
		case runtimev1.CommandStatusCreated, runtimev1.CommandStatusQueued, runtimev1.CommandStatusFailedRetryable:
	...
	if targetConn.GetState() != runtimev1.ConnStateConnected ||
...
func (a *V1RuntimeActionAdapter) QueryPlayback(ctx context.Context, petInstanceID string) (*behavior.PlaybackSnapshot, error) {
	if a.facade == nil {
...
func runtimePlaybackStatusOccupiesForeground(status string) bool {
	switch status {
	case runtimev1.PlaybackStatusPlaying, runtimev1.PlaybackStatusHolding, runtimev1.PlaybackStatusPaused:
		return true
	default:
		return false
	}
...
var _ behavior.RuntimeActionPort = (*V1RuntimeActionAdapter)(nil)
