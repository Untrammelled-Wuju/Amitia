...
func TestReconnectFencesSupersededConnectionBeforeCommandAckMutation(t *testing.T) {
	...
	cmd := &RuntimeCommand{
		...
		PlaybackRequestID:    "pbi-" + strings.TrimPrefix("cmd-reconnect-fence", "cmd-policy-"),
		PayloadJSON:          `{}`,
...
func TestPlayActionCompletionPolicyOnStartedCompletesAtStarted(t *testing.T) {
	db, services, handler := newHandlerP0DB(t)
	cmd := createPolicyCommand(t, db, "cmd-policy-started", PlayActionCompletionOnStarted, "pbi-started")
...
func TestPlayActionCompletionPolicyOnFirstCycleCompletesAtFirstCycle(t *testing.T) {
	db, services, handler := newHandlerP0DB(t)
	cmd := createPolicyCommand(t, db, "cmd-policy-cycle", PlayActionCompletionOnFirstCycle, "pbi-cycle")
...
func TestPlayActionInterruptionCancelsUnlessPolicyDeclaresSuccess(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			db, services, handler := newHandlerP0DB(t)
			cmd := createPolicyCommand(t, db, "cmd-policy-interrupt", tt.policy, "pbi-interrupt")
			payload, _ := json.Marshal(map[string]any{
				"commandId":          cmd.ID,
...
func createPolicyCommand(t *testing.T, db *gorm.DB, commandID, policy, playbackID string) *RuntimeCommand {
	t.Helper()
	now := time.Now().UTC().Format("2006-01-02 15:04:05")
