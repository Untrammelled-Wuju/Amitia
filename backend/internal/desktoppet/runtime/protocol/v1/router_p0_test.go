...
func TestParseRuntimeBootstrapSubprotocolRejectsAmbiguousOrMissingCredentials(t *testing.T) {
	tests := []struct {
		...
		{name: "valid", protocols: []string{runtimeV1WebSocketSubprotocol, runtimeV1BootstrapProtocolPrefix + "ticket-1"}, wantTicket: "ticket-1", wantOK: true},
		{name: "missing runtime protocol", protocols: []string{runtimeV1BootstrapProtocolPrefix + "ticket-1"}, wantOK: false},
		{name: "missing bootstrap protocol", protocols: []string{runtimeV1WebSocketSubprotocol}, wantOK: false},
		{name: "ambiguous bootstrap protocols", protocols: []string{runtimeV1WebSocketSubprotocol, runtimeV1BootstrapProtocolPrefix + "ticket-1", runtimeV1BootstrapProtocolPrefix + "ticket-2"}, wantOK: false},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantOK {
				if selected != runtimeV1WebSocketSubprotocol || ticket != tc.wantTicket {
...
func TestRuntimeWebSocketEnforcesRegistrationTimeoutAndMessageLimit(t *testing.T) {
	t.Run("registration timeout", func(t *testing.T) {
		...
		dialer := websocket.Dialer{Subprotocols: []string{
			runtimeV1WebSocketSubprotocol,
			runtimeV1BootstrapProtocolPrefix + "timeout-ticket",
		}}
	...
	t.Run("message size limit", func(t *testing.T) {
		...
		dialer := websocket.Dialer{Subprotocols: []string{
			runtimeV1WebSocketSubprotocol,
			runtimeV1BootstrapProtocolPrefix + "limit-ticket",
		}}
		conn, _, err := dialer.Dial(wsURL, nil)
		if err != nil {
			t.Fatal(err)
...
func TestRuntimeWebSocketTicketHelloAndIdentityHardBinding(t *testing.T) {
	...
	plainReq.Header.Set("Sec-WebSocket-Protocol", runtimeV1WebSocketSubprotocol+", "+runtimeV1BootstrapProtocolPrefix+"ticket-1")
	...
	dialer := websocket.Dialer{Subprotocols: []string{
		runtimeV1WebSocketSubprotocol,
		runtimeV1BootstrapProtocolPrefix + "ticket-1",
	}}
	conn, _, err := dialer.Dial(wsURL, nil)
	if err != nil {
		t.Fatal(err)
	...
	defer conn.Close()
	if conn.Subprotocol() != runtimeV1WebSocketSubprotocol {
