...
const (
	runtimeV1WebSocketSubprotocol     = "amitia.runtime.v1"
	runtimeV1BootstrapProtocolPrefix  = "amitia.runtime.bootstrap."
...
func RegisterInternalRoutes(
	...
	r.GET(path, internalOriginMiddleware(facade, safeMode), gin.WrapH(&runtimeWSHandler{facade: facade, consumeTicket: consumeTicket}))
}
...
type runtimeWSHandler struct {
	facade        *RuntimeFacade
	consumeTicket BootstrapTicketConsumer
	upgrader      websocket.Upgrader
}

func (h *runtimeWSHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if h.facade == nil || !h.facade.IsStarted() {
		http.Error(w, "runtime v1 not started", http.StatusServiceUnavailable)
		return
	}
	...
	if err != nil {
		...
		log.Warn("[runtime-v1-ws] upgrade failed: ", err)
		return
	}
	...
	dispatcher := NewConnectionCommandDispatcher(h.facade.Commands(), runtimeHandler)
...
func parseRuntimeBootstrapSubprotocol(r *http.Request) (selectedProtocol, rawTicket string, ok bool) {
	...
	for _, candidate := range websocket.Subprotocols(r) {
		...
		switch {
		case protocolName == runtimeV1WebSocketSubprotocol:
			hasRuntimeProtocol = true
		case strings.HasPrefix(protocolName, runtimeV1BootstrapProtocolPrefix):
	...
	rawTicket = strings.TrimSpace(strings.TrimPrefix(bootstrapProtocol, runtimeV1BootstrapProtocolPrefix))
	if rawTicket == "" {
		return "", "", false
	}
	return runtimeV1WebSocketSubprotocol, rawTicket, true
}
	...
	for {
		...
		if env.MessageType == MessageTypeHello {
			if err := env.ValidateBase(); err != nil {
				log.Warn("[runtime-v1-ws] hello base validation failed: ", err)
				return
			}
		} else {
			if err := env.ValidateEstablishedSession(); err != nil {
				log.Warn("[runtime-v1-ws] envelope session validation failed: ", err)
				return
			}
		}
		...
		if !env.VerifyPayloadHash() {
			log.Warn("[runtime-v1-ws] payload hash mismatch: type=", env.MessageType, " clientHash=", env.PayloadHash, " serverHash=", protocol.ComputePayloadHash(env.Payload))
			return
		}
		if env.MessageType != MessageTypeHello {
		...
		case MessageTypeHello:
			...
			if err := json.Unmarshal(env.Payload, &payload); err != nil {
				log.Warn("[runtime-v1-ws] hello payload unmarshal failed: ", err)
				return
			}
			ack, err := ctx.handler.HandleHello(ctx.conn, &payload)
			if err != nil || ack == nil {
				log.Warn("[runtime-v1-ws] hello rejected: ", err)
				return
			}
			if ctx.config != nil {
