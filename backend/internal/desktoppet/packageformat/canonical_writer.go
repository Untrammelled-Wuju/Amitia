...
func (w *CanonicalWriter) SchemaVersion() int { return 1 }

func (w *CanonicalWriter) WriteManifest(manifest *Manifest) ([]byte, error) {
	_, data, err := w.FinalizeManifest(manifest)
	return data, err
}

func (w *CanonicalWriter) FinalizeManifest(manifest *Manifest) (*Manifest, []byte, error) {
	if manifest == nil {
	...
	clone.Integrity.Algorithm = IntegrityAlgorithmV1
...
func (w *CanonicalWriter) WriteManifestToFile(manifest *Manifest, path string) error {
