...
func (r *ArchiveReader) ReadArchive(path string) (*Manifest, []byte, error) {
	...
	if err != nil {
		return nil, nil, NewPackageError(ErrCodePackagePathInvalid, fmt.Sprintf("archive not found: %s", path), err)
	}
	if info.Size() > r.limits.MaxCompressedSize {
		return nil, nil, NewPackageError(ErrCodePackageArchiveBomb, "compressed size exceeds limit", nil)
	}

	zipReader, err := zip.OpenReader(path)
	if err != nil {
		return nil, nil, NewPackageError(ErrCodePackageManifestInvalid, "failed to open zip archive", err)
	}
	...
	for _, f := range zipReader.File {
		...
		if fileCount > r.limits.MaxFileCount {
			return nil, nil, NewPackageError(ErrCodePackageArchiveBomb, "file count exceeds limit", nil)
		}
		...
		if len(normalized) > r.limits.MaxPathLength {
			return nil, nil, NewPackageError(ErrCodePackagePathInvalid, fmt.Sprintf("path too long: %s", normalized), nil)
		}

		depth := strings.Count(normalized, "/") + 1
		if depth > r.limits.MaxDirDepth {
			return nil, nil, NewPackageError(ErrCodePackagePathInvalid, fmt.Sprintf("directory depth exceeds limit: %s", normalized), nil)
		}

		if f.UncompressedSize64 > uint64(r.limits.MaxSingleFileSize) {
			return nil, nil, NewPackageError(ErrCodePackageArchiveBomb, fmt.Sprintf("single file too large: %s", normalized), nil)
		}

		totalUncompressed += int64(f.UncompressedSize64)
		if totalUncompressed > r.limits.MaxUncompressedSize {
			return nil, nil, NewPackageError(ErrCodePackageArchiveBomb, "total uncompressed size exceeds limit", nil)
		}
		...
		if r.limits.MaxCompressionRatio > 0 && f.CompressedSize64 > 0 && f.UncompressedSize64 > 0 {
			...
			if ratio > r.limits.MaxCompressionRatio {
				return nil, nil, NewPackageError(ErrCodePackageArchiveBomb, fmt.Sprintf("compression ratio too high: %s (%.1f:1)", normalized, ratio), nil)
			}
		}
		...
		if normalized == archiveManifestName {
			rc, openErr := f.Open()
			if openErr != nil {
				return nil, nil, NewPackageError(ErrCodePackageManifestInvalid, "failed to open manifest in archive", openErr)
			}
			data, readErr := io.ReadAll(rc)
			rc.Close()
			if readErr != nil {
				return nil, nil, NewPackageError(ErrCodePackageManifestInvalid, "failed to read manifest from archive", readErr)
			}
			manifestData = data
		}
	}
	...
	if manifestData == nil {
		return nil, nil, NewPackageError(ErrCodePackageManifestInvalid, "manifest.json not found in archive", nil)
	}
