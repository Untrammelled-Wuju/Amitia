...
const defaultNegativeConstraint = "多余人物，多余肢体，手脚数量异常，脸部变形，服装变化，发型变化，角色裁切，角色超出画布，视角突变，比例突变，背景复杂，文字，水印，标志，分镜边框，漫画格，多个重复角色，只画上半身，缺少腿部，缺少双脚，半身特写"
...
func buildSpriteSheetRules() string {
	...
	"每格只出现一个完整角色，必须是全身像：头部到双脚全部完整可见，双脚踩在格子底部区域，禁止只画上半身",
	"必须生成完整的12帧循环待机动画，所有格子共同构成一个连续流畅的动作循环",
	"最后一格必须与第一格完全相同：相同的姿势、相同的肢体位置、相同的比例，实现首尾帧无缝衔接",
	"角色完整可见，位于格内中心，四周保留少量空白边距，不得触碰格子边缘",
	"相邻格表示动作的连续进度，动作过渡自然流畅",
