...
func (s *SQLiteOutboxStore) ClaimNext(batchSize int, owner string) ([]OutboxRecord, error) {
	...
	err := s.db.Where("status = ? AND available_at <= ?", string(OutboxStatusPending), availableAt).
		Limit(batchSize).
		Find(&models).Error
	if err != nil {
		return nil, err
	...
	for _, m := range models {
		res := s.db.Model(&OutboxRecordModel{}).
			Where("id = ? AND status = ?", m.ID, string(OutboxStatusPending)).
...
func (s *SQLiteOutboxStore) MarkFailed(id, leaseToken, errMsg string) error {
	...
	if err := s.db.Where("id = ?", id).Take(&m).Error; err != nil {
		return err
	}
