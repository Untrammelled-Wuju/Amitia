...
func TestStartupOrderingDrill(t *testing.T) {
	...
	if len(results) != 6 {
		t.Fatalf("expected 6 phases, got %d", len(results))
	}
	if !seq.AllReady(results) {
		t.Fatal("expected ready gate passed")
	}
}
...
func TestDrainShutdownDrill(t *testing.T) {
	...
	if !lc.drainCalled {
		t.Fatal("drain should be called")
	}
}
...
func TestForceShutdownDrill(t *testing.T) {
	...
	results := order.Execute(ctx)
	if results[0].Error == "" {
		t.Fatal("expected error from forced drain timeout")
	}
}
...
func TestRestartRecoveryDrill(t *testing.T) {
	...
	if len(results1) != 2 {
		t.Fatalf("first run: expected 2 phases, got %d", len(results1))
	}
	results2 := seq.Execute(ctx)
	if len(results2) != 2 {
		t.Fatalf("restart: expected 2 phases, got %d", len(results2))
	}
}
func TestOutboxLeaseReturnDrill(t *testing.T) {
	...
	lease.Return()
	if lease.Active {
		t.Fatal("returned lease should be inactive")
	}
}
...
func TestExpiredResultDiscardDrill(t *testing.T) {
	...
	if !result.IsExpired() {
		t.Fatal("past-deadline result should be expired")
	}
	result.Discard()
	if !result.Discarded {
		t.Fatal("expired result should be discarded")
	}
}
...
func TestBasicChatConsistencyDrill(t *testing.T) {
	...
	if chat.MessageCount != 2 {
		t.Fatalf("expected 2 messages, got %d", chat.MessageCount)
	}
}
...
func TestStateConsistencyDrill(t *testing.T) {
	...
	if !state.IsConsistent() {
		t.Fatal("monotonic increments should be consistent")
	}
	if !state.ValidateVersion(5) {
		t.Fatal("newer version should be valid")
	}
	if state.ValidateVersion(2) {
		t.Fatal("older version should be invalid")
	}
}
...
func TestUnknownDeliveryDrill(t *testing.T) {
	...
	del.Retry()
	if del.Attempts != 1 {
		t.Fatalf("expected 1 attempt, got %d", del.Attempts)
	}
	del.Fail()
	del.Retry()
	del.Fail()
	...
	if del.Status != "failed" {
		t.Fatalf("expected failed, got %s", del.Status)
	}
}
...
func TestToolSideEffectCompensationDrill(t *testing.T) {
	...
	if comp.Applied {
		t.Fatal("compensation should start unapplied")
	}
	comp.Apply()
	if !comp.Applied {
		t.Fatal("compensation should be applied")
	}
	comp.Revert()
	if comp.Applied {
		t.Fatal("reverted compensation should not be applied")
	}
}
...
func TestMultiCircuitBreakerDrill(t *testing.T) {
	...
	for _, name := range names {
		reg.Register(name, DefaultCircuitBreakerConfig())
	}
	for _, name := range names {
		for i := 0; i < DefaultCircuitBreakerConfig().FailureThreshold; i++ {
			reg.RecordFailure(name)
		}
		if cb := reg.Get(name); cb.Status() != CircuitOpen {
			t.Fatalf("%s expected open, got %s", name, cb.Status())
		}
	}
	reg.ResetAll()
	for _, name := range names {
		if cb := reg.Get(name); cb.Status() != CircuitClosed {
			t.Fatalf("%s expected closed after reset, got %s", name, cb.Status())
		}
	}
}
...
func (m *mockStartupComponent) Startup(ctx context.Context) error {
	if m.fail {
...
func (l *OutboxLease) IsActive() bool {
	if !l.Active || time.Now().UTC().After(l.ExpiresAt) {
		l.Active = false
		return false
	}
	return true
