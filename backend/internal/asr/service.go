...
func (s *service) Update(id int, updates map[string]interface{}) (*AsrConfig, error) {
	...
	if cfg != nil { cfg.HasApiKey = cfg.ApiKey != ""; cfg.ApiKey = "" }
	return cfg, nil
}
...
func (s *service) Activate(id int) (*AsrConfig, error) {
	if err := s.repo.Activate(id); err != nil { return nil, fmt.Errorf("激活失败: %w", err) }
	cfg, _ := s.repo.GetByID(id)
	if cfg != nil { cfg.HasApiKey = cfg.ApiKey != ""; cfg.ApiKey = "" }
	return cfg, nil
}
...
func (s *service) GetActiveApiKey() (string, error) {
	...
	if err != nil { return "", fmt.Errorf("没有激活的ASR配置") }
	return cfg.ApiKey, nil
}

func (s *service) TestConnection(id int) error {
	cfg, err := s.repo.GetByID(id)
	if err != nil { return fmt.Errorf("ASR配置不存在") }
	if cfg.ApiKey == "" { return fmt.Errorf("API Key未设置") }
	return nil
}
