...
func TemporalRelationshipTimeMigration() Migration {
	return Migration{
		...
		Up: func(s *Step) error {
			ownerColumn := "user_id"
			if hasSpace, err := s.ColumnExists("relationship_states", "space_id"); err != nil {
				return err
			} else if hasSpace {
				ownerColumn = "space_id"
			}
			render := func(sql string) string {
				if ownerColumn == "user_id" {
					return sql
				}
				return strings.ReplaceAll(sql, "user_id", "space_id")
			}
			s.Execute(render(`INSERT INTO relationship_states (
id,
character_id,
relation_type,
relation_data,
...
AND NOT EXISTS (
    ...
    AND canonical.relation_type = 'user_character'
			...
			s.Execute(render(`CREATE UNIQUE INDEX IF NOT EXISTS idx_relationship_canonical_scope
ON relationship_states(user_id, character_id, channel, relation_type)
WHERE channel = '*' AND relation_type = 'user_character'`))
			s.CreateTable(render(`CREATE TABLE IF NOT EXISTS temporal_global_presence_states (
...
updated_at_utc TEXT NOT NULL DEFAULT ''
			...
			s.CreateTable(render(`CREATE TABLE IF NOT EXISTS temporal_relationship_presence_states (
id TEXT PRIMARY KEY,
...
updated_at_utc TEXT NOT NULL DEFAULT ''
			...
			s.CreateTable(render(`CREATE TABLE IF NOT EXISTS temporal_cadence_samples (
id TEXT PRIMARY KEY,
user_id TEXT NOT NULL DEFAULT 'default',
character_id TEXT NOT NULL DEFAULT '',
...
created_at_utc TEXT NOT NULL DEFAULT ''
			...
			s.CreateTable(render(`CREATE TABLE IF NOT EXISTS temporal_reunion_episodes (
id TEXT PRIMARY KEY,
user_id TEXT NOT NULL DEFAULT 'default',
character_id TEXT NOT NULL DEFAULT '',
...
updated_at_utc TEXT NOT NULL DEFAULT ''
			...
			s.CreateTable(render(`CREATE TABLE IF NOT EXISTS temporal_interaction_receipts (
id TEXT PRIMARY KEY,
...
updated_at_utc TEXT NOT NULL DEFAULT ''
			...
			s.CreateTable(render(`CREATE TABLE IF NOT EXISTS temporal_effect_ledger (
id TEXT PRIMARY KEY,
...
applied_at_utc TEXT NOT NULL DEFAULT ''
			...
			s.CreateTable(render(`CREATE TABLE IF NOT EXISTS temporal_relationship_time_settings (
...
updated_at_utc TEXT NOT NULL DEFAULT ''
			...
			if err := s.CreateIndex("idx_temporal_relationship_presence_scope", "temporal_relationship_presence_states", []string{render("user_id"), "character_id"}, true); err != nil {
				return err
			}
			...
			if err := s.CreateIndex("idx_temporal_cadence_scope_time", "temporal_cadence_samples", []string{render("user_id"), "character_id", "sample_kind", "current_interaction_at_utc"}, false); err != nil {
				return err
			}
			if err := s.CreateIndex("idx_temporal_reunion_idempotency", "temporal_reunion_episodes", []string{"idempotency_key"}, true); err != nil {
				return err
			}
			if err := s.CreateIndex("idx_temporal_reunion_scope_time", "temporal_reunion_episodes", []string{render("user_id"), "character_id", "detected_at_utc"}, false); err != nil {
				return err
			}
			if err := s.CreateIndex("idx_temporal_receipt_request", "temporal_interaction_receipts", []string{render("user_id"), "request_id"}, true); err != nil {
				return err
			}
