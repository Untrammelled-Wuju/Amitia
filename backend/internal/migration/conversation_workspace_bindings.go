...
func ConversationWorkspaceBindingsBaselineRepairMigration() Migration {
	return Migration{
		Version: "20260915001",
		Name:    "repair_conversation_workspace_bindings_baseline_drift",
		Up: func(s *Step) error {
			s.CreateTable(`CREATE TABLE IF NOT EXISTS conversation_workspace_bindings (
				conversation_id TEXT PRIMARY KEY,
				workspace_id TEXT NOT NULL,
			...
			if err := s.CreateIndex("idx_conversation_workspace_bindings_workspace", "conversation_workspace_bindings", []string{"workspace_id"}, false); err != nil {
				return err
			}
			return s.CreateIndex("idx_conversation_workspace_bindings_device", "conversation_workspace_bindings", []string{"device_id"}, false)
		},
	}
}
