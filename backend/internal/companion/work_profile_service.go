...
func parseWorkProfileUpdates(body map[string]interface{}) map[string]interface{} {
	...
	if v, ok := body["enabled"].(bool); ok {
		if v { updates["enabled"] = 1 } else { updates["enabled"] = 0 }
	}
	...
	if v, ok := body["allowOvertime"].(bool); ok {
		if v { updates["allow_overtime"] = 1 } else { updates["allow_overtime"] = 0 }
	}
	...
	if v, ok := body["delayedReplyEnabled"].(bool); ok {
		if v { updates["delayed_reply_enabled"] = 1 } else { updates["delayed_reply_enabled"] = 0 }
	}
	if v, ok := body["commuteHomeShareEnabled"].(bool); ok {
		if v { updates["commute_home_share_enabled"] = 1 } else { updates["commute_home_share_enabled"] = 0 }
	}
