...
func setupCompanionScopeService(t *testing.T) *service {
	...
	statements := []string{
		...
		"CREATE TABLE active_message_settings (id INTEGER PRIMARY KEY AUTOINCREMENT, character_id TEXT DEFAULT '', enabled INTEGER DEFAULT 1, active_level INTEGER DEFAULT 50, min_interval INTEGER DEFAULT 60, quiet_start TEXT DEFAULT '23:00', quiet_end TEXT DEFAULT '07:00', max_per_day INTEGER DEFAULT 6, max_daily_calls INTEGER DEFAULT 10, channel TEXT DEFAULT 'all', unreplied_slowdown_enabled INTEGER DEFAULT 1, unreplied_slowdown_after INTEGER DEFAULT 2, unreplied_cooldown_multiplier REAL DEFAULT 2.0, unreplied_recovery_on_reply INTEGER DEFAULT 1, created_at TEXT DEFAULT '', updated_at TEXT DEFAULT '')",
