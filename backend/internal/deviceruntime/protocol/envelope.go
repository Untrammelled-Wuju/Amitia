...
func (e *Envelope) ValidateBase(descriptor Descriptor) error {
	if e.EnvelopeVersion != descriptor.EnvelopeVersion {
		return fmt.Errorf("unsupported envelope version: %d", e.EnvelopeVersion)
	}
	if e.Protocol != descriptor.Name {
		return fmt.Errorf("unexpected protocol: %s", e.Protocol)
	}
	if !e.MessageType.IsValid() {
...
func (e *Envelope) ValidateEstablishedSession(descriptor Descriptor) error {
	if err := e.ValidateBase(descriptor); err != nil {
		return err
	}
...
func (e *Envelope) Validate(descriptor Descriptor) error {
	return e.ValidateEstablishedSession(descriptor)
}
