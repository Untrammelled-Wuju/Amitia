...
object QQBotChatBridgeContract {
    const val INPUT = "each inbound message starts one command immediately"
    const val OUTPUT = "one assistant turn streams one continuous text block"
    const val TOOLING = "reasoning, tools and rich output are structured blocks"
}
`
