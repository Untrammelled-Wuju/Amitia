...
func (r *repository) List(q MemoryListQuery) ([]Memory, int64, error) {
	...
	if q.MemoryType != "" { query = query.Where("memory_type = ?", q.MemoryType) }
	if q.Keyword != "" { query = query.Where("(key LIKE ? OR value LIKE ?)", "%"+q.Keyword+"%", "%"+q.Keyword+"%") }
	var total int64
	query.Count(&total)
	if q.Page <= 0 { q.Page = 1 }
	if q.PageSize <= 0 { q.PageSize = 50 }
	sortBy := q.SortBy
	if sortBy == "" || !map[string]bool{"updated_at": true, "created_at": true, "importance": true, "use_count": true}[sortBy] { sortBy = "updated_at" }
	var items []Memory
	err := query.Order(sortBy + " DESC").Offset((q.Page - 1) * q.PageSize).Limit(q.PageSize).Find(&items).Error
	if items == nil { items = []Memory{} }
...
func (r *repository) RecordUse(id string) error {
	return r.db.Model(&Memory{}).Where("id = ?", id).Updates(map[string]interface{}{"use_count": gorm.Expr("use_count + 1"), "last_used_at": gorm.Expr("datetime('now')")}).Error
}
