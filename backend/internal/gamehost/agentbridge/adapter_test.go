...
func TestSessionRegistryDefaultAgentContextEnrichesColdPluginSessions(t *testing.T) {
	...
	scope, ok := sessions.Resolve("runtime-1", "plugin-session-1")
	if !ok {
...
func TestSessionRegistryRetainRuntimesPrunesRemovedContext(t *testing.T) {
	sessions := NewSessionRegistry()
	sessions.Bind(SessionScope{RuntimeID: "runtime-keep", PluginID: "plugin-a", CharacterID: "char-a"})
	sessions.Bind(SessionScope{RuntimeID: "runtime-remove", PluginID: "plugin-b", CharacterID: "char-b"})
	sessions.RetainRuntimes([]ghdomain.RuntimeInstanceID{"runtime-keep"})
	if _, ok := sessions.Resolve("runtime-remove", ""); ok {
		t.Fatal("removed runtime context must be pruned")
	}
	if scope, ok := sessions.Resolve("runtime-keep", ""); !ok || scope.CharacterID != "char-a" {
