...
func (a *OpenAIResponsesAdapter) Protocol() chat.ModelProtocol {
	return chat.ProtocolOpenAIResponses
}

func (a *OpenAIResponsesAdapter) Capabilities(ctx context.Context, cfg *chat.ModelConfig) chat.ModelCapabilities {
...
func (a *OpenAIResponsesAdapter) Generate(ctx context.Context, cfg *chat.ModelConfig, req chat.ModelRequest) (*chat.ModelResult, error) {
...
func (a *OpenAIResponsesAdapter) Stream(ctx context.Context, cfg *chat.ModelConfig, req chat.ModelRequest, sink chat.ModelEventSink) (*chat.ModelResult, error) {
	baseURL := strings.TrimRight(cfg.BaseURL, "/")
...
func (a *OpenAIResponsesAdapter) buildInput(req chat.ModelRequest) []map[string]interface{} {
...
func (a *OpenAIResponsesAdapter) buildTools(tools []chat.ModelToolDefinition) []map[string]interface{} {
...
func (a *OpenAIResponsesAdapter) parseResponse(respBytes []byte) (*chat.ModelResult, error) {
	var result struct {
		ID     string `json:"id"`
		Output []struct {
	...
	for _, item := range result.Output {
		...
		case "reasoning":
			if res.Continuation == nil {
				res.Continuation = &chat.ModelContinuationState{
					Protocol: string(chat.ProtocolOpenAIResponses),
				}
			}
			if item.Summary != "" {
...
func (a *OpenAIResponsesAdapter) parseStream(body io.Reader, sink chat.ModelEventSink) (*chat.ModelResult, error) {
		...
		if n > 0 {
			...
			for _, line := range lines {
				...
				case "response.output_text.done":
					sink.Emit(context.Background(), chat.ModelEvent{
						Type: chat.ModelEventTextDone,
					})
				case "response.function_call_arguments.delta":
					sink.Emit(context.Background(), chat.ModelEvent{
						Type:           chat.ModelEventToolCallArgumentsDelta,
				...
				case "response.reasoning_summary_text.delta":
					sink.Emit(context.Background(), chat.ModelEvent{
						Type:      chat.ModelEventReasoningSummaryDelta,
						TextDelta: event.Delta,
					})
				...
				case "response.failed":
					sink.Emit(context.Background(), chat.ModelEvent{
						...
						Error: &chat.ModelError{
							...
							Protocol: chat.ProtocolOpenAIResponses,
