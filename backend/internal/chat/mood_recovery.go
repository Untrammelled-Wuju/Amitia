...
func (s *service) moodRecoveryCheck(convID, charID, source string) {
	if source == "proactive" || source == "system" { return }
	var lastAt string
	...
	if idleDur > 6*time.Hour {
		...
		s.db.Exec("INSERT INTO mood_logs (character_id, mood, intensity, source, note, created_at) VALUES (?, 'happy', 7, 'reconnect', ?, datetime('now'))", charID, fmt.Sprintf("用户回来了 (冷落%d小时)", idleHours))
	}
}