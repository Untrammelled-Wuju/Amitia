...
func (h *Handler) CreateConversation(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil { util.ErrorResponse(c, response.InvalidParams, "缺少必要参数", nil); return }
...
func (h *Handler) DeleteConversation(c *gin.Context) {
	id := c.Param("id")
	if err := h.service.DeleteConversation(id); err != nil { util.ErrorResponse(c, response.InternalError, "删除失败", nil); return }
	util.SuccessMsgResponse(c, "对话已删除", nil)
}

func (h *Handler) DeleteAllConversations(c *gin.Context) {
	if err := h.service.DeleteAllConversations(); err != nil { util.ErrorResponse(c, response.InternalError, "删除失败", nil); return }
	util.SuccessMsgResponse(c, "所有对话已删除", nil)
}

func (h *Handler) DeleteMessages(c *gin.Context) {
	id := c.Param("id")
	if err := h.service.DeleteMessages(id); err != nil { util.ErrorResponse(c, response.InternalError, "清空失败", nil); return }
	util.SuccessMsgResponse(c, "消息已清空", nil)
}

func (h *Handler) DeleteSingleMessage(c *gin.Context) {
	id := c.Param("id")
	if err := h.service.DeleteSingleMessage(id); err != nil { util.ErrorResponse(c, response.NotFound, err.Error(), nil); return }
...
func (h *Handler) SearchMessages(c *gin.Context) {
	...
	c.ShouldBindQuery(&q)
	if q.Keyword == "" { util.ErrorResponse(c, response.InvalidParams, "关键词不能为空", nil); return }
	resp, err := h.service.SearchMessages(q)
	if err != nil { util.ErrorResponse(c, response.InternalError, "搜索失败", nil); return }
	util.SuccessResponse(c, resp)
}

func (h *Handler) ChangeCharacter(c *gin.Context) {
	id := c.Param("id")
	var body struct{ CharacterID string `json:"characterId"` }
	if err := c.ShouldBindJSON(&body); err != nil || body.CharacterID == "" { util.ErrorResponse(c, response.InvalidParams, "characterId 不能为空", nil); return }
...
func (h *Handler) Chat(c *gin.Context) {
	...
	if err != nil { util.ErrorResponse(c, response.BusinessError, err.Error(), nil); return }
	util.SuccessResponse(c, resp)
}
...
func (h *Handler) CreateModel(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&raw); err != nil { util.ErrorResponse(c, response.InvalidParams, "无效请求体", nil); return }
	cfg := ModelConfig{}
	if v, ok := raw["apiType"]; ok { cfg.APIType, _ = v.(string) }
	if v, ok := raw["baseUrl"]; ok { cfg.BaseURL, _ = v.(string) }
	if v, ok := raw["apiKey"]; ok { cfg.APIKey, _ = v.(string) }
	if v, ok := raw["modelName"]; ok { cfg.ModelName, _ = v.(string) }
	if v, ok := raw["name"]; ok { cfg.Name, _ = v.(string) }
	if v, ok := raw["isActive"]; ok {
		switch val := v.(type) {
		case bool:
			if val { cfg.IsActive = 1 } else { cfg.IsActive = 0 }
		case float64:
			if val != 0 { cfg.IsActive = 1 } else { cfg.IsActive = 0 }
		case int:
			cfg.IsActive = val
		}
	}
	if cfg.APIType == "" { cfg.APIType = "openai-compatible" }
...
func (h *Handler) DeleteModel(c *gin.Context) {
	id, _ := strconv.Atoi(c.Param("id"))
	if err := h.service.DeleteModel(id); err != nil { util.ErrorResponse(c, response.OperationFailed, "删除失败", nil); return }
...
func (h *Handler) UpdateModelRoutes(c *gin.Context) {
	var body struct{ Routes []map[string]interface{} `json:"routes"` }
	if err := c.ShouldBindJSON(&body); err != nil { util.ErrorResponse(c, response.InvalidParams, "无效请求体", nil); return }
	if err := h.service.UpdateModelRoutes(body.Routes); err != nil { util.ErrorResponse(c, response.OperationFailed, err.Error(), nil); return }
...
func (h *Handler) DetectModels(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&body); err != nil || body.BaseURL == "" { util.ErrorResponse(c, response.InvalidParams, "baseUrl 不能为空", nil); return }
	models, err := h.service.DetectModels(body.BaseURL, body.APIKey)
	if err != nil { util.ErrorResponse(c, response.BusinessError, err.Error(), nil); return }
	if models == nil { models = []ModelDetectItem{} }
	util.SuccessResponse(c, gin.H{"models": models})
}

func (h *Handler) GetSummary(c *gin.Context) { util.SuccessResponse(c, gin.H{"summary": "", "conversationId": c.Param("id")}) }
...
func (h *Handler) GetModel(c *gin.Context) {
	...
	for _, m := range models { if m.ID == id { m.HasAPIKey = m.APIKey != ""; util.SuccessResponse(c, m); return } }
...
func (h *Handler) ProviderSchema(c *gin.Context) { util.SuccessResponse(c, gin.H{"fields": []string{"baseUrl", "apiKey", "modelName"}}) }
...
func (h *Handler) doTestConnection(c *gin.Context, baseURL, apiKey, modelName string) {
	...
	if resp.StatusCode == 200 || resp.StatusCode == 401 || resp.StatusCode == 403 {
		msg := "连接成功"
		if resp.StatusCode == 401 || resp.StatusCode == 403 { msg = "服务可达，请检查 API Key" }
		util.SuccessResponse(c, gin.H{
