...
func RegisterRouter(group *gin.RouterGroup, ctx *app.AppContext, runtime *Runtime) {
	...
	packageHandler := NewPackageHandler(runtime.Packages, handler)
	...
	extensions.GET("/packages/metrics", packageHandler.Metrics)
	extensions.GET("/package-operations", packageHandler.Operations)
	extensions.GET("/package-operations/:operationId", packageHandler.Operation)
	extensions.GET("/signers", packageHandler.Signers)
	extensions.POST("/signers/:fingerprint/trust", packageHandler.TrustSigner)
	extensions.POST("/signers/:fingerprint/untrust", packageHandler.UntrustSigner)
	extensions.GET("/:id/exports/:exportId", packageHandler.Download)
	extensions.POST("/:id/export", packageHandler.Export)
	extensions.POST("/:id/upgrade/preview", packageHandler.PreviewUpgrade)
	extensions.POST("/:id/upgrade", packageHandler.Upgrade)
	extensions.GET("/:id/versions", packageHandler.Versions)
	extensions.GET("/:id/versions/compare", packageHandler.Compare)
	extensions.POST("/:id/versions/:version/rollback", packageHandler.Rollback)
	extensions.GET("/:id/dependencies", packageHandler.Dependencies)
	extensions.GET("/:id/uninstall/preview", packageHandler.PreviewUninstall)
	extensions.DELETE("/:id", packageHandler.Uninstall)
