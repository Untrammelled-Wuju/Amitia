...
func (s *PackageService) DetectLegacyPackages(ctx context.Context) (LegacyMigrationReport, error) {
	...
	if s == nil || s.repository == nil || s.repository.db == nil {
		return report, fmt.Errorf("legacy package migration repository unavailable")
	}
	if s.kernelProxy == nil {
		return report, fmt.Errorf("extension kernel migration unavailable")
	}
	var candidates []legacyPackageMigrationCandidate
	err := s.repository.db.WithContext(ctx).Raw(`
		SELECT e.extension_id, e.current_version AS version, COALESCE(v.package_blob, X'') AS package_blob,
	...
	for _, candidate := range candidates {
		...
		if found && (state == LegacyMigrationCompleted || state == LegacyMigrationManualRequired || state == LegacyMigrationPendingManual) {
		...
		report.PendingManual++
		report.PendingExtensions = append(report.PendingExtensions, candidate.ExtensionID)
		if err := s.kernelProxy.recordLegacyMigrationState(ctx, legacyMigrationRecord{ExtensionID: candidate.ExtensionID, State: LegacyMigrationPendingManual, Source: source}); err != nil {
			return report, err
		}
	}
	return report, nil
