...
func validateDeveloperSessionBinding(
	...
	sessionID, spaceID, extensionID string,
) error {
	...
	if sessionID == "" || spaceID == "" || extensionID == "" || sessions == nil || workspaces == nil {
	...
	if session.SpaceID != spaceID || string(session.ExtensionID) != extensionID || session.PolicyVersion != packagePolicyVersion || session.Environment != "development" || !session.DevTrustSnapshot {
	...
	if workspace.OwnerSpaceID != spaceID || string(workspace.ExtensionID) != extensionID || !workspace.DevTrust || workspace.DevTrustVersion != session.DevTrustVersion {
