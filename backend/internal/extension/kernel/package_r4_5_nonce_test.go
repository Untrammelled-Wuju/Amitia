...
func r45NonceBindingFixture(nonce, operationType, extensionID, spaceID string, issuedAtUnix, expiresAtUnix int64) PackageConfirmationNonceBinding {
...
func TestR45NonceConsumeRejectsMissingIssuedAt(t *testing.T) {
	...
	binding.IssuedAt = ""
	op := PackageOperationRecord{OperationID: "op-1", OperationType: "install", ExtensionID: "ext-1", SpaceID: "user-1",
...
func TestR45NonceConsumePersistsCompleteBinding(t *testing.T) {
	...
	op := PackageOperationRecord{OperationID: "op-persist", OperationType: "install", ExtensionID: "ext-persist", SpaceID: "user-persist",
	...
	var record struct {
		nonce, operationID, operationType, extensionID, spaceID, issuedAt, expiresAt, consumedAt string
	}
	row := repo.db.QueryRow(`SELECT nonce, operation_id, operation_type, extension_id, space_id, issued_at, expires_at, consumed_at FROM extension_package_confirmation_nonces WHERE operation_id=?`, existing.OperationID)
	if err := row.Scan(&record.nonce, &record.operationID, &record.operationType, &record.extensionID, &record.spaceID, &record.issuedAt, &record.expiresAt, &record.consumedAt); err != nil {
	...
	if record.extensionID != binding.ExtensionID || record.spaceID != binding.SpaceID {
...
func TestR45NonceReplayAcrossOperationsReturnsTokenStale(t *testing.T) {
	...
	op1 := PackageOperationRecord{OperationID: "op-1", OperationType: "install", ExtensionID: "ext-1", SpaceID: "user-1",
		IdempotencyKey: "key-1", RequestHash: "hash-1", AttemptCount: 1}
	if _, _, err := repo.CreateOrGetOperationWithConfirmationNonce(context.Background(), op1, binding); err != nil {
		t.Fatalf("first create should succeed: %v", err)
	}
	op2 := PackageOperationRecord{OperationID: "op-2", OperationType: "install", ExtensionID: "ext-1", SpaceID: "user-1",
...
func TestR45NonceReplayAcrossUsersReturnsTokenStale(t *testing.T) {
	...
	binding2 := binding1
	binding2.SpaceID = "user-2"
	op2 := PackageOperationRecord{OperationID: "op-2", OperationType: "install", ExtensionID: "ext-1", SpaceID: "user-2",
		IdempotencyKey: "key-2", RequestHash: "hash-2", AttemptCount: 1}
...
func TestR45NonceReplayAcrossExtensionsReturnsTokenStale(t *testing.T) {
	...
	op2 := PackageOperationRecord{OperationID: "op-2", OperationType: "install", ExtensionID: "ext-2", SpaceID: "user-1",
		IdempotencyKey: "key-2", RequestHash: "hash-2", AttemptCount: 1}
	if _, _, err := repo.CreateOrGetOperationWithConfirmationNonce(context.Background(), op2, binding2); !IsPackageOperationError(err, OperationErrTokenStale) {
...
func TestR45NonceReplayAcrossOperationTypesReturnsTokenStale(t *testing.T) {
	...
	op2 := PackageOperationRecord{OperationID: "op-2", OperationType: "update", ExtensionID: "ext-1", SpaceID: "user-1",
		IdempotencyKey: "key-2", RequestHash: "hash-2", AttemptCount: 1}
	if _, _, err := repo.CreateOrGetOperationWithConfirmationNonce(context.Background(), op2, binding2); !IsPackageOperationError(err, OperationErrTokenStale) {
	...
	for i := 0; i < workers; i++ {
		...
		go func(idx int) {
			...
			op := PackageOperationRecord{
				...
				OperationType: "install", ExtensionID: "ext-concurrent", SpaceID: "user-concurrent",
