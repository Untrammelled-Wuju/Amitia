...
func buildIOSToolDefinitions() []capability.ToolDefinition {
	...
	defs := []capability.ToolDefinition{
		...
		// === SHORTCUTS (truthful native surface) ===
		...
		makeIOSNativeTool(rt, shortcuts.ToolIDShortcutsProvider, "", shortcuts.OperationShortcutsProvider, "Get Shortcuts Provider", "Get the native App Shortcuts provider phrases", genericInput, genericOutput, []capability.PermissionRequirement{{Capability: "ios.shortcuts.execute", Risk: "medium"}}, capability.RiskMedium, false),
		...
		// === ALARMS (8 tools; countdown/pause/resume require a Live Activity widget extension) ===
