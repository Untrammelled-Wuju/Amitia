...
func resolveSnapshotOwner(db *sql.DB, conversationID, characterID string) string {
	if db != nil {
		if conversationID = strings.TrimSpace(conversationID); conversationID != "" {
			var owner string
			if err := db.QueryRow("SELECT user_id FROM conversations WHERE id = ? AND deleted_at IS NULL", conversationID).Scan(&owner); err == nil {
		...
		if characterID = strings.TrimSpace(characterID); characterID != "" {
			var owner string
			if err := db.QueryRow("SELECT user_id FROM characters WHERE id = ?", characterID).Scan(&owner); err == nil {
				if owner = strings.TrimSpace(owner); owner != "" {
					return owner
				}
			}
