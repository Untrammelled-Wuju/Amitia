...
func (r *CandidateRepository) Save(ctx context.Context, record *CandidateRecord) error {
	...
	promoteStartedAt := ""
	if !record.PromoteStartedAt.IsZero() {
		promoteStartedAt = record.PromoteStartedAt.Format(time.RFC3339Nano)
	}
	promoteCommittedAt := ""
	if !record.PromoteCommittedAt.IsZero() {
		promoteCommittedAt = record.PromoteCommittedAt.Format(time.RFC3339Nano)
	}
	rollbackStartedAt := ""
	if !record.RollbackStartedAt.IsZero() {
		rollbackStartedAt = record.RollbackStartedAt.Format(time.RFC3339Nano)
	}
	rollbackFinishedAt := ""
	if !record.RollbackFinishedAt.IsZero() {
		rollbackFinishedAt = record.RollbackFinishedAt.Format(time.RFC3339Nano)
	}
	_, err = r.db.ExecContext(ctx, `
		...
		(candidate_id, extension_id, instance_ids_json, generation_id, candidate_generation,
		 ...
		 promote_started_at, promote_committed_at, rollback_started_at, rollback_finished_at, module_id)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		record.CandidateID,
...
func (r *CandidateRepository) UpdatePromoteStarted(ctx context.Context, candidateID string, startedAt time.Time) error {
	if r == nil || r.db == nil {
		return nil
	}
	_, err := r.db.ExecContext(ctx, `
		UPDATE kernel_candidate_contributions
		SET promote_started_at = ?, updated_at = ?
		WHERE candidate_id = ?`,
	...
	if err != nil {
		return fmt.Errorf("candidate-repo: update promote_started_at %s: %w", candidateID, err)
	}
	return nil
}

func (r *CandidateRepository) UpdatePromoteCommitted(ctx context.Context, candidateID string, committedAt time.Time) error {
	if r == nil || r.db == nil {
		return nil
	}
	_, err := r.db.ExecContext(ctx, `
		UPDATE kernel_candidate_contributions
		SET promote_committed_at = ?, updated_at = ?
		WHERE candidate_id = ?`,
	...
	if err != nil {
		return fmt.Errorf("candidate-repo: update promote_committed_at %s: %w", candidateID, err)
	}
	return nil
}
...
func (r *CandidateRepository) ListByStatus(ctx context.Context, status CandidateStatus) ([]*CandidateRecord, error) {
	if r == nil || r.db == nil {
		return nil, nil
	}
	rows, err := r.db.QueryContext(ctx, `
	...
	if err != nil {
		return nil, fmt.Errorf("candidate-repo: list by status %s: %w", status, err)
	}
	defer rows.Close()
...
func scanCandidateRecord(rows *sql.Rows) (*CandidateRecord, error) {
	var (
		...
		createdAtStr        string
		updatedAtStr        string
		promoteStartedStr   string
		promoteCommittedStr string
		rollbackStartedStr  string
		rollbackFinishedStr string
	)
	if err := rows.Scan(
		&record.CandidateID,
		&extID,
	...
	if promoteStartedStr != "" {
		if t, err := time.Parse(time.RFC3339Nano, promoteStartedStr); err == nil {
			record.PromoteStartedAt = t
		}
	}
	if promoteCommittedStr != "" {
		if t, err := time.Parse(time.RFC3339Nano, promoteCommittedStr); err == nil {
			record.PromoteCommittedAt = t
		}
	}
	if rollbackStartedStr != "" {
		if t, err := time.Parse(time.RFC3339Nano, rollbackStartedStr); err == nil {
			record.RollbackStartedAt = t
		}
	}
	if rollbackFinishedStr != "" {
		if t, err := time.Parse(time.RFC3339Nano, rollbackFinishedStr); err == nil {
			record.RollbackFinishedAt = t
		}
	}
	if instanceIDsStr != "" {
