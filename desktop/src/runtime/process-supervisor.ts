import type { ManagedProcessDefinition, ManagedProcessStatus } from "../shared/types"
...
export class InMemoryProcessSupervisor implements ProcessSupervisor {
  ...
  async start(definition: ManagedProcessDefinition): Promise<ManagedProcessStatus> {
    const starting: ManagedProcessStatus = { id: definition.id, state: "starting" }
    this.statuses.set(definition.id, starting)
    const running: ManagedProcessStatus = { id: definition.id, state: "running", pid: 0 }
  ...
  fail(id: string, error: string, exitCode = 1): ManagedProcessStatus {
    const failed: ManagedProcessStatus = { id, state: "failed", error, exitCode }
